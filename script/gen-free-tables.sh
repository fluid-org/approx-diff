#!/usr/bin/env bash
# Compile the dump-free-tables Agda program and run it; the binary writes
# test-baselines/free-dependency.txt via Agda IO.
...
( cd agda && agda --compile --compile-dir=_build --ghc-flag=-rtsopts \
    ...
    src/graph-viz/dump-free-tables.agda > "$log" 2>&1 )
GHCRTS="${DUMP_GHCRTS:--M1G -s}" agda/_build/dump-free-tables

echo "wrote test-baselines/free-dependency.txt"
