...
programs. We relate the two categories at first-order type in \autoref{sec:definability}, which ensures that we will be able to derive reverse dependency maps even for programs that use higher order functions internally.}
...
products, because $\SDSemiMod(S)$ has products.} \Deleted{Our category of interest, $\Fam(\LatGal)$ has coproducts and finite
...
them have infinite products. \todo{cite: Adamek paper}. \Deleted{We
  ...
  will see in \autoref{sec:definability}, this is no problem for
