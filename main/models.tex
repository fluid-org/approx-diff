...
\Added{For basic differentiable programming, inputs and outputs are
  ...
  \Deleted{The previous section concluded that L-posets and stable functions give
...
\begin{example}
  ...
  \item \Deleted{In $\LatGal$, right adjoints are summed using meets and left
  ...
  \item \Deleted{$\MeetSLat$ and $\JoinSLat$ are both $\CMon$-enriched and have
    finite products similar to $\LatGal$.}
  \end{enumerate}
\end{example}

\begin{remark}
  A $\CMon$-category having biproducts means that its finite products
  and coproducts coincide, but this does not necessarily extend to
  infinite products and coproducts. $\SemiMod(S)$ has infinite
  products and coproducts, but these do not coincide. For this reason,
  $\SDSemiMod(S)$ does not have infinite products because the dual of
  an infinite product is no longer uniquely isomorphic to an infinite
  product of duals. This will become relevant when we consider
  Cartesian closure in \secref{models:Cartesian-closure}.
\end{remark}
...
\Added{As explained in \secref{approx-as-tangents:autodiff}, every point
...
\Deleted{L-posets are partially ordered sets where every principal downset
...
seen in \remref{chain-rule}\Deleted{, \remref{chain-rule-cm}, and
...
functions. \Deleted{We conjecture at the end of this section that the analogous
...
\Added{We will use the categories $\Fam(\SDSemiMod(S))$ as our model
...
\Added{Our category of interest, $\Fam(\SDSemiMod(S))$ has coproducts
...
\Deleted{Our category of interest, $\Fam(\LatGal)$ has coproducts and finite
...
construction to $\Fam(\cat{C})$. \Deleted{This relies on the
  definition of \emph{biproducts}, which we discuss below in
  \secref{biproducts}.}
...
associated linear map at every point\Deleted{, and when $\cat{C} = \LatGal$,
...
The category \Replaced{$\SemiMod(S)$}{$\MeetSLat$} satisfies the hypotheses of
\thmref{fam-closed}, so:
\begin{corollary}
  \Replaced{$\SemiMod(S)$}{$\Fam(\MeetSLat)$} is Cartesian closed and has all coproducts.
\end{corollary}
Unfortunately, neither \Replaced{$\SDSemiMod(S)$}{$\LatGal$} nor
$\FinVect$ satisfy the hypotheses of this theorem, because as we noted
above, neither of them have infinite products. \todo{cite: Adamek
  paper}. \Deleted{We will consider ways to rectify this below in
  \secref{fixing-completeness}.} \Added{So we are forced to use
...
\Added{As we will see in \secref{definability}, this is no problem
...
\begin{remark}
  ...
  (\remref{biproduct-not-closed}). \Deleted{We will implicitly use
  Hermida's construction in our definability proof in
  \secref{definability}, where we use a logical relations argument to
  show that every morphism definable in the higher order language is
  also first-order definable.}
\end{remark}
