...
\Added{For basic differentiable programming, inputs and outputs are
  ...
  the semiring settings identified in \secref{sec:approx-as-tangents}
...
As we noted at the end of \secref{sec:approx-as-tangents:semiring},
...
\begin{remark}
  ...
  Cartesian closure in \secref{sec:models:Cartesian-closure}.
\end{remark}

\begin{remark}
  \label{remark:biproduct-not-closed}
...
\Added{As explained in \secref{sec:approx-as-tangents:autodiff}, every point
...
\Added{We will use the categories $\Fam(\SDSemiMod(S))$ as our model
  ...
  category $\Fam(\SemiMod(S))$ to interpret higher order programs. We
  relate the two categories at first order type in
  \secref{sec:definability}, which ensures that we will be able to
  derive reverse dependency maps even for programs that use higher
  order functions internally.}
...
above, neither of them have infinite products. \todo{cite: Adamek
  ...
  transpose of linear maps afforded by $\Fam(\SDSemiMod(S))$. We can
  always embed interpretations of first-order programs in
  $\Fam(\SDSemiMod(S))$ into $\Fam(\SemiMod(S))$ via a functor:}
...
\begin{proposition}
  ...
  does) and all coproducts.
\end{proposition}

\Added{As we will see in \secref{sec:definability}, this is no problem
...
\begin{remark}
  ...
  (\remref{remark:biproduct-not-closed}). We will implicitly use
