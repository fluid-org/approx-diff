...
\Added{In the previous section, we saw several examples of commutative
  semirings that capture different ways of tracking how programs'
  outputs depend on their inputs. To apply these settings to a
  programming language we need an interpretation that attaches
  semimodules over our chosen semiring to the interpretation of types, and
  ...
  reals. All of the results of this section have been mechanised in
...
\Added{As explained in \autoref{sec:approx-as-tangents:autodiff}, every point
...
\Added{We will use the categories $\Fam(\SDSemiMod(S))$ as our model of
semiring tracking data provenance, analogous to V{\'a}k{\'a}r \emph{et
  al.}'s use of $\Fam(\FinVect)$. The self-duality of objects in
$\SDSemiMod(S)$ means that we can take transposes of tangent maps to
track reverse provenance. To interpret expressive languages in this
setting, we need to ensure it has sufficient categorical
structure. Unfortunately, it is the case that $\Fam(\SDSemiMod(S))$ is
not cartesian closed, and so cannot support an interpretation of
higher order functions. This is because $\SDSemiMod(S)$ does not have
infinite products, as we explain below. This motivates the use of the
more liberal category $\Fam(\SemiMod(S))$ to interpret higher order
programs. We relate the two categories at first order type in \autoref{sec:correctness}, which ensures that we will be able to derive reverse dependency maps even for programs that use higher order functions internally.}
...
\Added{Our category of interest, $\Fam(\SDSemiMod(S))$ has coproducts and finite
products, because $\SDSemiMod(S)$ has products.}
...
$\FinVect$ satisfy the hypotheses of this theorem, because neither of
them have infinite products. \todo{cite: Adamek paper}. \Deleted{We
  will consider ways to rectify this below in
  ...
  will see in \autoref{sec:correctness}, this is no problem for
