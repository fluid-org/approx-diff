...
forms part of a monad $L$ on \Replaced{$\LatConj$}{$\LatGal$}. This monad extends to a
(strong) monad $\Lift$ on $\Fam(\Replaced{\LatConj}{\LatGal})$ with
...
\begin{displaymath}
  ...
  \end{array}
\end{displaymath}
Examining the morphisms so defined in $\Fam(\Replaced{\LatConj}{\LatGal})$, we can see
...
\begin{displaymath}
  ...
  \end{array}
\end{displaymath}
\Deleted{In comparison to the categories $\CM$ and $\Lposet$ from
...
\begin{remark}
  ...
  These examples highlight a potential criticism of $\Fam(\Replaced{\LatConj}{\LatGal})$ as
  ...
  $\Fam(\Replaced{\LatConj}{\LatGal})$, of turning this into a rigorous statement that the
  ...
  morphisms\footnote{This additional structure is reminiscent of the
    ...
    original $x$. We observe that objects of $\Fam(\Replaced{\LatConj}{\LatGal})$ arising
...
of the terminal object in $\Fam(\Replaced{\LatConj}{\LatGal})$, yields an object that
