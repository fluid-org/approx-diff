...
\secref{stab}.}
Our working hypothesis is that \Replaced{$\Fam(\LatConj)$}{$\Fam(\LatGal)$}, where values and their
...
\thmref{fam-closed}, so:
\begin{corollary}
  \Replaced{$\Fam(\JoinSLat)$}{$\Fam(\MeetSLat)$} is Cartesian closed and has all coproducts.
\end{corollary}
...
\begin{corollary}
  ...
  \Replaced{$\Fam(\JoinSLat \times \JoinSLat^\op)$}{$\Fam(\MeetSLat \times \JoinSLat^\op)$} is Cartesian closed and has
  all coproducts.
\end{corollary}

This corollary means that, assuming a sensible intepretation of primitive types and operations, we
can use \Replaced{$\Fam(\JoinSLat \times \JoinSLat^\op)$}{$\Fam(\MeetSLat \times \JoinSLat^\op)$} to
interpret the higher-order language we describe in the next section. We still regard the category
\Replaced{$\Fam(\LatConj)$}{$\Fam(\LatGal)$} as the reference model of approximable sets with
forward and backward approximation maps; the category
\Replaced{$\Fam(\JoinSLat \times \JoinSLat^\op)$}{$\Fam(\MeetSLat \times \JoinSLat^\op)$} is a technical
device to carry out the interpretation of higher-order programs. To get interpretations of first-order types
and primitive operations, we can embed \Replaced{$\Fam(\LatConj)$}{$\Fam(\LatGal)$} into
\Replaced{$\Fam(\JoinSLat \times \JoinSLat^\op)$}{$\Fam(\MeetSLat \times \JoinSLat^\op)$}:

\begin{proposition}
  \label{prop:ho-embedding}
  The functor
  \Replaced{$\HoEmbed : \Fam(\LatConj) \to \Fam(\JoinSLat \times
  \JoinSLat^\op)$}{$\HoEmbed : \Fam(\LatGal) \to \Fam(\MeetSLat \times \JoinSLat^\op)$} is defined
  on objects as
...
\Replaced{$\Fam(\LatConj)$}{$\Fam(\LatGal)$} and
\Replaced{$\Fam(\JoinSLat \times \JoinSLat^\op)$}{$\Fam(\MeetSLat \times \JoinSLat^\op)$}, as long as we
interpret the base types as objects in \Replaced{$\Fam(\LatConj)$}{$\Fam(\LatGal)$}. At higher-order,
...
\Replaced{$\Fam(\LatConj)$}{$\Fam(\LatGal)$}.
