...
reveals new applications such as approximation by intervals~(\secref{interval-approx}). It also opens up an
opportunity to integrate traditional symbolic data provenance techniques with the AD-based methods used in
explainable AI, such as the saliency maps of Grad-CAM~\cite{selvaraju20}. Our categorical approach admits a
modular construction of the model, and the use of general theorems, such as Fiore and Simpson's
\thmref{glr-definability}, to prove properties of the interpretation. We have focused on constructions that
enable an executable implementation in Agda.}
...
\paragraph{Recursion and Partiality} \Deleted{This work has only examined the case for total programs. Even
though we took stable domain theory as our starting point, we did not make any use of directed completeness or
similar properties of domains. We expect that an account of recursion in an extension of the framework
discussed so far would likely involve families of bounded lattices indexed by DCPOs, where the order of the
DCPO would be reflected in embedding-projection relationships between the lattices. \citet{berry79}'s
bidomains and \citet{laird07}'s bistable biorders have separate extensional and stable orders on the same
set.} \Added{This work treats only total programs, and makes no use of directed completeness or similar
...
in \Replaced{some implementations of provenance}{earlier \GPS implementations}, would be to develop a
...
structured target language formalised as a Grothendieck construction. Adapting this to \Replaced{our setting
would allow the analysis to be}{Galois slicing would allow slicing to} ``compiled in'', avoiding the need for
