...
\paragraph[Shape Approximation]{\replaced{Shape Approximation}{General Inductive and Coinductive Types}}
...
adopt a similar approach. \Deleted{Full inductive types would allow us to embed an interpreter for a small
language; combined with the CBN monadic translation described in \secref{cbn-translation} which uniformly
inserts approximation points, we should be able to obtain the program slicing behaviour of earlier \GPS work
``for free''. Coinductive types (e.g.~streams) present additional challenges, especially for defining
...
\Added{Richer datatypes bring the question of \emph{shape approximation}: approximating not only the values
at the positions of a structure but the structure itself, such as the length of a list or the depth of a
tree. This was central to the earlier, operational presentations of Galois
slicing~\cite{perera12a,perera16d,ricciotti17}, whose approximation lattices represent partially erased data,
but is absent from our model, which approximates only values and holds the shape of each input fixed.
Bringing it into the present semantic setting would require the approximation object of a value to vary
with its shape rather than being fixed in advance, and would also let coinductive data such as streams be
sliced through their finite prefixes.}
...
\citet{laird07}'s bistable biorders have separate extensional and stable orders on the same set\Deleted{, in a
way that might be similar to \exref{intervals-and-maxima-elements}}.
