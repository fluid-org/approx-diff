...
$\Fam(\JoinSLat \times \JoinSLat^\op)$, so it is not a priori evident
that we get a conjugate pair from the interpretation of a program
...
\begin{theorem}
  ...
  there exists
  $g \in \Fam(\LatConj)(\sem{\Gamma}_{\mathit{fo}},
...
\begin{proof}
  ...
  $\HoEmbed : \Fam(\LatConj) \to \Fam(\JoinSLat \times
  ...
  $\Fam(\LatConj)$ such that $H(g) = f$. Since $\GLR(\HoEmbed)$ has
  ...
  $p(\sem{\Gamma \vdash M : \tau}_{\GLR(\HoEmbed)}) = \sem{\Gamma
    \vdash M : \tau}_{\Fam(\JoinSLat \times \JoinSLat^\op)}$ yields
  the result.
\end{proof}
...
\begin{remark}
  ...
  $\Fam(\LatConj)$, then if there is a finite bicartesian functor
  $\cat{G} \to \Fam(\JoinSLat \times \JoinSLat^\op)$, an analogous
...
higher-order interpretation in $\Fam(\JoinSLat \times \JoinSLat^\op)$
...
\begin{theorem}
  ...
  $\sem{\Gamma \vdash M : \tau}_{\Fam(\JoinSLat \times
...
\begin{proof}
  ...
  $\langle \mathrm{Id} , \pi_1 \rangle : \Fam(\JoinSLat \times
  \JoinSLat^\op) \to \Fam(\JoinSLat \times \JoinSLat^\op) \times \Set$
  ...
  $g = \sem{\Gamma \vdash M : \tau}_{\Fam(\JoinSLat \times
    \JoinSLat^\op)}$ and
