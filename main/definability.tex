...
\Deleted{As we noted at the end of \secref{language:gps-interpretation}, our
...
\begin{proof}
  Instantiate \thmref{glr-definability} with the functor
  $\langle \mathrm{Id} , \pi_1 \rangle : \Fam(\SemiMod(S)) \to
  \Fam(\SemiMod(S)) \times \Set$ that is the identity in the first
  component and projects out the underlying function in the
  second. For each $\Gamma \vdash M : \tau$, we obtain a $g$ such that
