...
\Added{Dependency analysis over the Booleans records whether an output can change when an input does; the
refund example of \secref{introduction:galois-slicing} raised the more fine-grained question of how much it
...
factors, with $\min$ as the addition and the numerical product as the multiplication. This is the
...
number, the catastrophic cancellation of numerical analysis. Where the Boolean instance could only report a
dependency that was not really there, this instance says how badly the bound degrades. For non-negative data
the factors are at most $1$, so the query examples remain exact. The two are simply different semirings, and
choosing between them is part of specifying what the analysis is to observe.}
