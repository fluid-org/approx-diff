...
is $\join$ and $\times$ is $\meet$, it is $(N \comp M)_{ji} = \bigjoin_k N_{jk} \meet M_{ki}$, which is
relational composition. The Boolean Jacobians are accordingly the morphisms of $\Mat(\Two)$, the category of
matrices over the Booleans, and since such a matrix is the same data as a relation between input and output
positions, $\Mat(\Two)$ is isomorphic to $\FinRel$, the category of finite sets and relations. Transposing a
...
\Replaced{Galois connections played a prominent role in the earlier work on program slicing: the forward map preserves meets, and the backward slicing map, recovering the least input responsible for a given output, is its adjoint, the two together forming a Galois connection.}{Our thesis is that \GPS is a generalised form of differentiable programming, where tangents are
not linear approximations of curves but instead are qualitative information approximations of elements.}
\Replaced{Not every function admits such a backward map. In analysis it is \emph{smoothness} that determines when a function can be differentiated; the qualitative counterpart is Berry's notion of a \emph{stable function}~\cite{berry79,berry82}, the condition under which a function's forward and backward maps form this connection, and hence under which the backward slice exists.}{Smooth functions in this setting are Berry's \emph{stable functions}
