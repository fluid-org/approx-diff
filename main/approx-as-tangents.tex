...
factors, with $\min$ and numerical $\times$; this is the multiplicative counterpart of the previous
presentation, related to it by logarithms over the reals, and the two are the classical dichotomy between
absolute and relative error~\cite{high:ASNA2}. Multiplication now has unit entries, since relative perturbations pass through a
...
\Added{At the refund example the amplification is $|3| / |3 + (-3)|$: division by zero, an infinite condition
number, the catastrophic cancellation of numerical analysis. The over-approximation that the Boolean instance
could only signal as a spurious dependency is here measured, and for nonnegative data the factors are at most
$1$, so the query examples remain exact. The choice between the two presentations, like the choice of semiring
itself, is part of specifying what the analysis is to observe.}
