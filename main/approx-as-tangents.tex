...
the backward derivative is its transpose. The real numbers play no role in this picture beyond supplying the
scalars. Our first move is therefore to keep the matrix algebra and change the scalars. Throughout this
...
\Added{Write $\Mat(S)$ for the category whose objects are natural numbers and whose morphisms $m \to n$ are
$n \times m$ matrices over $S$, with composition given by matrix multiplication $(NM)_{ij} = \sum_k N_{ik}
\cdot M_{kj}$ and identities by the Kronecker $\delta$. (Associativity uses distributivity of $\cdot$ over
$+$ and the interchange law for iterated sums, both consequences of the semiring axioms.) A vector $v \in
S^m$ is a morphism $1 \to m$, and decomposes as $v = \sum_i v_i \cdot e_i$, where $e_i$ is the $i$-th
standard basis vector. The Jacobians of \secref{introduction} live here: a dependency relation between $m$
input positions and $n$ output positions is exactly a morphism $m \to n$ in $\Mat(\Two)$, where $\Two$ is
the Boolean semiring with $+ = \vee$ and $\cdot = \wedge$, and the chain rule for dependencies is
composition in $\Mat(\Two)$.}
...
general (arbitrary self-dual semimodules, or a Chu-style category of pairs), with $\Mat(S)$ as the concrete
instance the examples run on? If we stick with $\Mat(S)$, we should say how it fits into the more general
picture.}
