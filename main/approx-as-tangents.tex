...
\Replaced{linear algebra}{stable domain theory} to reconstruct the \Replaced{dependency analyses of
\secref{introduction}}{ideas of \GPS} in a denotational setting.
...
\Added{In the Euclidean case, the differential picture above is plain matrix algebra: with a chosen basis,
...
\Added{Our second move is one of abstraction: rather than axiomatise the matrices of \secref{introduction}
...
\Added{The transpose is almost the backward analysis we are after: it runs in the opposite direction to $f$,
...
\end{definition}
\end{AddedBlock}
\Added{The pairing is an $S$-valued measure of the extent to which $x$ and $y$ overlap; its prototype is the
...
\end{definition}
\end{AddedBlock}
\Added{The conjugate is our backward analysis, and its defining equation makes precise the relationship with
the forward one asserted in \secref{introduction}. Self-dual semimodules and linear maps form a category
$\SDSemiMod(S)$.}

\Added{The matrices of \secref{introduction} now reappear as concrete instances. The \emph{free} semimodule
...
\Added{We write $\Mat(S)$ for the skeletal category whose objects are the natural numbers and whose morphisms
...
\Added{The trivial semimodule is a zero object in $\SDSemiMod(S)$, and self-dual semimodules are closed under
...
\subsection[Dependency Structures over Richer Semirings]{\added{Dependency Structures over Richer Semirings}}
\label{sec:approx-as-tangents:richer-semirings}

\Added{The framework so far only asks for commutativity of the scalars, which makes the dual $M^*$
...
\subsubsection[Conjugate pairs and conjugacy analyses]{\added{Conjugate pairs and conjugacy analyses}}

\Added{Suppose first that $1$ is an additive top in $S$ ($1 + a = 1$). Addition is then
...
\Added{If multiplication is also idempotent ($a \cdot a = a$), then $S$ is a bounded distributive lattice,
with multiplication as the meet and distributivity given by the semiring. This opens the way to reading the
two analyses in terms of \emph{disjointness}: where meets exist, say $x \disj y$ when $x \meet y = 0$, and
...
\end{definition}
\end{AddedBlock}
\Added{The notion originates in relation algebra~\cite{jonsson51}, where the conjugate of composition with a
relation is composition with its converse. The scalars, viewed as a semimodule over themselves, qualify
automatically for this reading, and biproducts (including the empty biproduct $S^0$) inherit the structure
componentwise, giving the linear maps between free semimodules $S^n$, with their conjugates, as the canonical
instances of conjugate pairs.}

\Added{The relevance to us here is that conjugate pairs also underpin the \emph{cognacy} (common ancestry)
...
\subsubsection[Galois slicing]{\added{Galois slicing}}

\Added{Suppose in addition that every scalar has a complement: an operation $\neg$ with $a \meet \neg a =
0$ and $a \join \neg a = 1$, making $S$ a Boolean algebra. Negation supports an alternative presentation of
conjugates as adjoints: composing $\conj{f}$ with complements on either side, giving its \emph{De Morgan
dual} $\neg \comp \conj{f} \comp \neg$, yields the upper adjoint of $f$, so forward and backward analysis
form a \emph{Galois connection} between the lattices of approximations.}
\begin{AddedBlock}
...
\end{definition}
\end{AddedBlock}
\Added{This is precisely the setting of
\GPS~\cite{perera12a,perera16d,ricciotti17}, recovered here as the Boolean instance of the general semimodule picture.
Galois connections as such presuppose none of the algebraic structure of this section: they make sense between
arbitrary posets. The Boolean algebra and the De Morgan
dual are simply how the adjoint presentation arises in the present framework, where the conjugate is the
primitive notion. Notably, in that line of work the forward map, a meet-preserving right adjoint, serves only to establish the
existence of the join-preserving backward map, and plays no direct role in the slicing or dependency analyses
themselves. The two presentations answer different questions: the conjugate propagates \emph{necessity} (which
inputs may the output depend on), while its De Morgan dual propagates \emph{sufficiency} (which inputs are
enough to determine the output); we return to this reading below.}
...
connections \added{(\defref{galois-connection})}, which will be more useful for what follows. This
characterisation is due to~\citet{taylor99}. \Deleted{We first define Galois
