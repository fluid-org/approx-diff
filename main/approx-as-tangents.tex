...
\linearto S$ of linear maps into the scalars, and every linear map $f : M \linearto N$ has a \emph{transpose}
$\transpose{f} = (- \comp f) : N^* \linearto M^*$, contravariantly.}
...
relation is composition with its converse. The relevance to us here is that conjugate pairs also underpin the
\emph{cognacy} (common ancestry) analyses of work on linked visualisations~\cite{perera22,bond25}: composing a
join-preserving dependency analysis with its conjugate yields a map sending a selection of outputs to its
\emph{related outputs}, those sharing a dependency on some part of the input; selecting marks in one chart
then highlights the cognate marks in another. Over a bounded distributive lattice, then, forward and backward
dependency analysis form a conjugate pair. The scalars, viewed as a semimodule over themselves, qualify
...
\Added{Suppose now that $S$ is a Boolean algebra. Negation converts conjugates into adjoints: composing
$\conj{f}$ with complements on either side yields the upper adjoint of $f$, and forward and backward analysis
form a Galois connection between the lattices of approximations. This is precisely the setting of \GPS,
recovered here as the Boolean instance of the general story. The two presentations answer
different questions: the conjugate propagates \emph{necessity} (which inputs may the output depend on), while
its De Morgan dual propagates \emph{sufficiency} (which inputs are enough to determine the output); we return
to this reading below.}
