...
dot product of vectors (the pairing induced by the self-duality of $S^n$, as we will see below).}
\begin{AddedBlock}
...
\subsection{Dependency Structures over Richer Semirings}
\label{sec:approx-as-tangents:richer-semirings}

\Added{The framework so far asks very little of the scalars: any commutative semiring $S$ yields a category
$\SDSemiMod(S)$ with a forward derivative and its conjugate, related through the pairing. This is where the
reals sit, with the conjugate as the familiar adjoint of linear algebra; addition of real numbers is not
idempotent, and no order structure is available. Additional axioms on $S$ equips the analysis with additional
structure.}

\Added{Suppose first that $S$ is \emph{additively bounded}: $1 + x = 1$, and hence $x + x = x$. Then every
semimodule carries a partial order, $x \leq y$ iff $x + y = y$, making it a join-semilattice with joins given
by $+$ and least element $0$, and every linear map automatically preserves joins and $0$. Dependency
information is now ordered by informativeness: $0$ means ``no dependency'', and the forward and backward
analyses are monotone, taking more approximate inputs to more approximate outputs.}

\Added{Suppose further that $S$ is a bounded distributive lattice, so that multiplication is a meet. The
lattice structure of $S$ does not lift to every semimodule, but it does equip the basic building blocks: the
scalars themselves, the trivial semimodule, and biproducts of such. Call two elements of a join-semilattice
\emph{disjoint} when their meet is $0$; on these building blocks, disjointness agrees with orthogonality of
the pairing, $x \meet y = 0$ iff $\langle x, y \rangle = 0$. The defining equation of the conjugate then
specialises to an exchange of disjointness: $\langle f(x), y \rangle = 0$ iff $\langle x, \conj{f}(y)
\rangle = 0$, so $f$ and $\conj{f}$ form a \emph{conjugate pair} in the sense of Tarski's relation
algebra~\cite{jonsson51}, each transporting non-overlap across the map.}

\Added{Finally, suppose $S$ is a Boolean algebra. Negation converts conjugates into adjoints: composing
