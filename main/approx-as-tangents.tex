...
$i$-th standard basis vector. The Boolean Jacobians from \secref{introduction} live here: a dependency
...
categories $\Mat(\Two) \simeq \FinRel$.}
...
\transpose{N}$, using commutativity of $\cdot$. $(-)^{\top}$ is thus a dagger on $\Mat(S)$. The transpose
can also be characterised equationally, in terms of the \emph{dot product} $\langle u, v \rangle = \sum_i
u_i \cdot v_i$: it is the unique matrix satisfying}
\cbstart
\begin{displaymath}
  \added{\langle M u, v \rangle = \langle u, \transpose{M} v \rangle}
\end{displaymath}
\cbend
\Added{uniqueness holding because pairing with the basis vectors extracts components, $\langle e_i, v
\rangle = v_i$. This makes precise the relationship between the forward and backward analyses asserted in
\secref{introduction}. Over $\Two$ the dot product has a direct reading: $\langle u, v \rangle = \top$
...
natural number $0$ is a zero object, and $m + n$ is a \emph{biproduct} of $m$ and $n$, an object which is both
a product and a coproduct, with the injections the transposes of the projections, $\biinj_i =
\transpose{\biproj_i}$. The coincidence of products and coproducts is characteristic of such linear settings,
and is the reason $\Mat(S)$ on its own cannot interpret programs over sum types; we return to this in
\secref{models-of-total-gps}.}
...
a pair of a forward $f : M \to M'$ and backward $g : N' \to N$ with $k'(f\,m, n') = k(m, g\,n')$.}
