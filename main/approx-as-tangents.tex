...
We motivate our approach by showing how to combine ideas from differential geometry\Added{, Boolean-valued dependency analysis,} and stable domain theory to reconstruct the ideas of \GPS in a denotational setting.
...
\begin{remark}[Chain Rule]
  ...
  \end{itemize}
  \Added{In the Euclidean case, where these maps are Jacobian matrices, the chain rule is matrix multiplication: the Jacobian of $g \circ f$ is the product of the Jacobians of $g$ and $f$, and the backward derivative is the product of their transposes in the reverse order.}
  ...
  derivative maps of $g \circ f$. As we shall see below, \Added{dependency maps and} stable maps
...
\cbstart
\subsection{Dependency Analysis as Differentiation}
\label{sec:dependency-as-differentiation}

\added{Our thesis is that dependency analysis of the Galois slicing variety is a form of \emph{qualitative
differentiation} in which the real scalars are replaced by Booleans. For data provenance, we are interested in
how a program, viewed as a function, depends on its input. At its most basic, we can ask whether it depends on
its input at all: for a function $f : A \to B$ we define its \emph{derivative} at $a \in A$ to be the truth
value
\[ f'(a) = \exists a' \in A.\ f(a) \neq f(a'), \] which is $\bot$ when $f$ is constant and $\top$ when its
output can change as its input varies, the qualitative analogue of a derivative being nonzero. This definition
doesn't actually depend on $a$, but if we consider $n$-ary functions $f : A_1 \times \cdots \times A_n \to B$,
we can define the \emph{partial derivative} at input position $i$ as
\[ \partial_i f(a_1, \ldots, a_n) = \exists a'.\ f(a_1, \ldots, a_i, \ldots, a_n) \neq f(a_1, \ldots, a',
\ldots, a_n), \] and the partial derivatives at a point form a vector of truth values, a qualitative analogue
of the \emph{gradient}. For integer multiplication, for instance, the gradient is $(\top, \top)$ wherever both
arguments are nonzero; at $(0, a)$ with $a$ nonzero it is $(\top, \bot)$, at $(a, 0)$ it is $(\bot, \top)$,
and at $(0, 0)$ it is $(\bot, \bot)$. For a function
with several outputs, $f : A_1 \times \cdots \times A_n \to B_1 \times \cdots \times B_m$, the partial
derivative of each output with respect to each input forms a matrix of truth values, the \emph{Boolean
Jacobian}, whose $(j, i)$ entries record whether output $j$ depends on input $i$, just as the ordinary Jacobian
of \secref{approx-as-tangents:autodiff} assembles the partial derivatives of a smooth function.}

\added{Just as the Jacobian of a smooth function acts on tangent spaces, the Boolean Jacobian acts on the
\emph{approximations} of a value. An approximation of a value records, at each of its positions, whether the data there is retained ($\top$) or
discarded ($\bot$), the $\bot$s being exactly the approximated-away data of \secref{introduction:galois-slicing}.
Such an approximation is thus a vector of truth values over the positions, or equivalently the subset of
positions marked $\top$, so the approximations form the powerset of the positions: a finite Boolean algebra
in which the Booleans $\mathbbm{2}$ play the role of the scalars $\RR$, and the individual positions the role
of a basis. The Boolean Jacobian is bare incidence data, the dependency relation between input and output
positions, and as in \secref{introduction:galois-slicing} it can be lifted to a map on approximations in two
ways; we take the join-preserving one:}
\begin{itemize}[leftmargin=\enummargin]
\item \added{The \emph{forward map} is the join-preserving (conjugate) map from approximations of the input to approximations of the output represented by the Boolean Jacobian.}
\item \added{The \emph{backward map} is the join-preserving map from approximations of the output to approximations of the input represented by its transpose.}
\end{itemize}

\begin{remark}[Chain Rule]
  \label{rem:chain-rule-bool}
  \added{Dependency maps compose by the chain rule, just as derivatives do: the Boolean Jacobian of a composite is the matrix product of the Boolean Jacobians, with forward maps composing in order and backward maps in reverse. As with smooth functions, this lets us compute the dependency maps of a composite program from those of its parts.}
\end{remark}

\added{Both cases are matrix multiplication over a semiring, $(NM)_{ji} = \sum_k N_{jk} M_{ki}$. Over the
reals this is the Jacobian product of \secref{approx-as-tangents:autodiff}; over the Booleans, where $+$
is $\join$ and $\times$ is $\meet$, it is $(N \comp M)_{ji} = \bigjoin_k N_{jk} \meet M_{ki}$. The Boolean
Jacobians are accordingly the morphisms of $\Mat(\Two)$, the category of
matrices over the Booleans, and since such a matrix is the same data as a relation between input and output
positions, $\Mat(\Two)$ is isomorphic to $\FinRel$, the category of finite sets and relations, with matrix
multiplication corresponding to composition of relations. Transposing a
matrix inverts the relation, which is how the backward map runs dependencies from outputs back to inputs;
this is the depends-on direction in which we drew the relation in \secref{introduction:galois-slicing}, and
for dependency analysis it is arguably the backward map that is primary, with the forward map obtained by
transposition.
Composing two
slicing functions thus chains their dependency relations: an output of the composite depends on an input
exactly when some intermediate position carries the dependency across.}

\added{In what sense does the transpose relate the forward and backward maps? For matrices over the reals,
transposition is characterised by the inner product identity $\langle y, Mx \rangle = \langle
\transpose{M}y, x \rangle$. Over the Booleans, the structure the transpose preserves is instead
\emph{disjointness}: writing $x \disjoint y$ when $x \meet y = \bot$, the relationship between the two maps is
that of a \emph{conjugate pair}, which we used informally in \secref{introduction:galois-slicing}.}

\begin{definition}[Conjugate pair]
  \label{def:conjugate-pair}
  \added{Suppose $X$ and $Y$ are bounded lattices. A \emph{conjugate pair} $(f, g) : X \to Y$ is a pair of
  join-preserving functions $f : X \to Y$ and $g : Y \to X$ satisfying $y \disjoint f(x) \iff g(y) \disjoint
  x$ for any $x \in X$ and $y \in Y$~\cite{jonsson51}.}
\end{definition}

\begin{lemma}
  \label{lem:transpose-conjugate}
  \added{For a Boolean matrix $M$, the maps $x \mapsto Mx$ and $y \mapsto \transpose{M}y$ form a conjugate
  pair $\Two^n \to \Two^m$, where $(Mx)_j = \bigjoin_i M_{ji} \meet x_i$ is the application of a matrix to a
  vector.}
\end{lemma}

\begin{proof}
  \added{Write $u \cdot v = \bigjoin_k u_k \meet v_k$ for the Boolean dot product, and note that vectors are
  disjoint exactly when their dot product is $\bot$. Then $y \cdot Mx = \bigjoin_{j,i}\, y_j \meet M_{ji}
  \meet x_i = \transpose{M}y \cdot x$, so both $y \disjoint Mx$ and $\transpose{M}y \disjoint x$ say that no
  retained output position is related by $M$ to a retained input position.}
\end{proof}

\added{The Galois adjoint stands in a different relationship to the matrix. Since the backward map preserves joins, it has a right adjoint,
namely the meet-preserving sufficiency map of
\secref{introduction:galois-slicing}; but unfolding the definition shows this adjoint to be $x \mapsto (j
\mapsto \bigmeet_i \neg M_{ji} \join x_i)$, the action not of $\transpose{M}$ but of the negated matrix $\neg
M$, multiplied out over the opposite semiring in which $+$ is $\meet$ and $\times$ is $\join$. Deriving the
adjoint from the matrix thus requires negating its entries, possible for Boolean scalars but not in a general
lattice of approximations, whereas the conjugate is generated by the transpose alone. Comparing the two formulas, negating the input and
output of the adjoint gives exactly the conjugate: the two forward maps are De Morgan duals.}
\cbend

\subsection{Stable Functions as Differentiable Functions}

\Added{\todo{Revise to de-emphasise significance of stable functions. Perhaps the important point to retain is
that stability/meet-preservation characterise when a function is ``sequential'' enough to have a unique
backwards derivative, but lose the suggestion that a meet-preserving forward map is significant as a
dependency analysis.}}
...
\begin{example}[Intervals and Maximal Elements]
  ...
  \Added{Being meet-preserving, this addition is a forward
  ``sufficiency'' map in the sense of
  \secref{introduction:galois-slicing}; we contrast it with the
  join-preserving ``dependency'' map in \secref{language:examples}.}
...
\begin{remark}[Chain Rule]
  ...
  \end{itemize}
  \Deleted{We will use this property to show that $\Lposet$ embeds
  into our category of sets-with-approximation.}
\end{remark}
...
\Added{\todo{Revise to de-emphasise the significance of stable functions, and revisit
\conref{tangent-stable-fns} in the light of the conjugate reframing.}}
