...
\Added{Rather than axiomatise the matrices of \secref{introduction} directly, we identify the minimal
structure required to capture what made that story work: a space of dependency information for each value,
...
spaces the forward map runs it through, as the transposed matrix did in \secref{introduction}. For that, each space must be identified with its dual.
Whereas a finite-dimensional vector space always admits such an identification (choose a basis), a semimodule
need not. We therefore make the identification part of the data. A \emph{self-dual} semimodule is a pair
$\langle M, i \rangle$ of a semimodule and a chosen isomorphism $i : M \cong M^*$, which induces a pairing
$\langle x, y \rangle = i(x)(y)$. Between self-dual semimodules $\langle M, i \rangle$ and $\langle N, j
\rangle$, every linear map $f : M \linearto N$ then has a \emph{conjugate} $\conj{f} = i^{-1} \comp
\transpose{f} \comp j : N \linearto M$, the unique linear map satisfying}
\cbstart
\begin{displaymath}
...
\Added{The canonical instances are the \emph{free} finite-dimensional semimodules $S^n$, whose elements are
$n$-length vectors of scalars, with standard basis vectors $e_i$ and every $v \in S^n$ decomposing as $v =
\sum_i v_i \cdot e_i$. Currying the \emph{dot product} $\langle u, v \rangle = \sum_i u_i \cdot v_i$ gives
