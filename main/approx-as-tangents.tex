...
\cong M^*$, which induces a pairing $\langle x, y \rangle = i(x)(y)$. Between self-dual semimodules $\langle
...
\Added{The matrices of \secref{introduction} now reappear as the canonical instances. The \emph{free}
finite-dimensional semimodule $S^n$ has as elements the $n$-length vectors of scalars, with standard basis
vectors $e_i$ and every $v \in S^n$ decomposing as $v = \sum_i v_i \cdot e_i$. Currying the \emph{dot product} $\langle u, v \rangle = \sum_i u_i \cdot v_i$ gives
