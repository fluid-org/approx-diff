...
\transpose{N}$, using commutativity of $\cdot$. $(-)^{\top}$ is thus a dagger on $\Mat(S)$. The transpose is
characterised by the \emph{dot product} $\langle u, v \rangle = \sum_i u_i \cdot v_i$: it is the unique matrix
satisfying}
\cbstart
\begin{displaymath}
