...
$\downset{Q}$.} \Replaced{The present work is instead denotational, recasting Galois slicing as
differentiation over a semiring; the Boolean case recovers it, and other semirings give further analyses.
Galois slicing is also more general than our framework in two respects that we plan to consider in future work
(\secref{conclusion}): it computes \emph{program slices} from approximation lattices representing partially
erased programs, and it approximates the \emph{shape} of data rather than only the values at fixed
...
the cognacy of \secref{examples:linked}, computed here by composing a Boolean Jacobian with its transpose.}
