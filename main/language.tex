...
of our chosen signature in $\Fam(\LatConj)$. Signatures are first-order, so it does not matter that
$\Fam(\LatConj)$ is not closed. Any such interpretation can be transported to
$\Fam(\JoinSLat \times \JoinSLat^\op)$ along the functor $H$ from \propref{ho-embedding} because it preserves
finite products. We can then interpret the whole language in $\Fam(\JoinSLat \times \JoinSLat^\op)$.

Interpreting a whole program $\Gamma \vdash t : \tau$ yield morphisms in
$\Fam(\JoinSLat \times \JoinSLat^\op)$ which, as in \secref{semantic-gps}, are triples
$(f, \partial f_f, \partial f_r)$ of the underlying function and the forward and backward approximation
maps. However, unlike in $\Fam(\LatConj)$, it is not guaranteed that the forward and backward maps even operate on
the same lattices, let alone form a conjugate pair. \lemref{first-order-agreement-types} guarantees that
the lattices agree, but the fact that they form a conjugate pair is less trivial. We will prove this
property in \secref{definability}.

% Given a model of $\Sigma$ in $\Fam(\LatConj)$, the category $\Fam(\JoinSLat \times \JoinSLat^\op)$ can
% implement $\Sem$. It is bicartesian closed by \corref{mslat-jslat-bcc}, and can model $\tyList$ using infinite
% coproducts (\secref{models-of-total-gps:coproducts-and-products}). The embedding $\HoEmbed: \Fam(\LatConj) \to
% \Fam(\JoinSLat \times \JoinSLat^\op)$ from \propref{ho-embedding} preserves finite products, and so can
% transport each $\sem{\rho}_{\PrimTy}$ and $\sem{\phi}_{\Op}$ into the higher-order setting to provide a model
% of $\Sigma$. Finally the lifting monad in $\Fam(\LatConj)$ (\secref{first-order:lifting}) is also preserved by
...
$\tmInr{\tmUnit}$. We consider several interpretations of $\Sigma_{\mathsf{num}}$ in $\Fam(\LatConj)$ and their
...
\item \Added{\todo{This interpretation uses the lifting monad, which does not transfer to $\LatConj$ (see
  \secref{models-of-total-gps:lifting}); revisit.}}We take $\sem{\mathsf{num}}_{\PrimTy} = \Lift(\Disc(\mathbb{R}))$, using the lifting monad from
...
\item Quantitative approximation information with non-trivial dependency can be obtained by using the interval
  ...
  \Added{For slicing, the forward map is the join-preserving conjugate, which differs from the
  meet-preserving map of \exref{intervals-and-maxima-elements} by exchanging $\sqcap$ and $\sqcup$:}
  \cbstart
  \begin{displaymath}
    \transpose{\mathrm{add}} =
  ...
  \end{displaymath}
  \cbend
  \Added{Suppose we approximate the first ``$\mathsf{a}$'' row by $[-\frac{1}{2},0]$ and the second by
  $[\frac{4}{5},\frac{3}{2}]$, discarding the ``$\mathsf{b}$'' row. Each contributes a translate of its
  interval to the output, $[\frac{1}{2},1]$ and $[\frac{4}{5},\frac{3}{2}]$ respectively, and the conjugate
  forward map, written $(\partial\mathrm{query})_{\hat{f}}$ as in \secref{introduction:galois-slicing}, joins
  the contributions by intersecting them:}
  \cbstart
  \begin{displaymath}
    (\partial\mathrm{query})_{\hat{f}}(\mathsf{a},\mathit{db})(\cdot,
    (\cdot,[-\tfrac{1}{2},0]),(\cdot,\bot),(\cdot,[\tfrac{4}{5},\tfrac{3}{2}]), \cdot) = [\tfrac{4}{5},1]
  \end{displaymath}
  \cbend
\end{enumerate}

We have tested these examples on our Agda implementation. See the
\Replaced{files \texttt{example.agda} and \texttt{example-intervals.agda}}{file \texttt{example.agda}}.
...
interpretation in $\Fam(\JoinSLat \times \JoinSLat^\op)$, the type of the reverse approximation map as a
