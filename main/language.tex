...
\item Quantitative approximation information with non-trivial dependency can be obtained by using the interval
  ...
  \Added{In the forward direction we have the two readings contrasted in
  \exref{intervals-and-maxima-elements}. The forward map of the addition morphism in
  \secref{interval-approx} is the meet-preserving sufficiency map; its conjugate, the join-preserving
  dependency map, is obtained by exchanging $\sqcap$ and $\sqcup$:}
  \cbstart
  \begin{displaymath}
    \transpose{\mathrm{add}} =
