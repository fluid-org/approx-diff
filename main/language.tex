...
\Deleted{First, we note the type of the reverse approximation map in this case. The type of the approximation
...
\end{displaymath}
\end{DeletedBlock}
\Deleted{in the category $\JoinSLat$, where $\partial\sem{\mathsf{num}}(x)$ is the lattice of
...
\item \Deleted{Quantitative approximation information with non-trivial dependency can be obtained by using
...
\begin{DeletedBlock}
  ...
  \end{displaymath}
\end{DeletedBlock}
  \Deleted{Thus, to achieve an output within $[\frac{9}{10},\frac{11}{10}]$, either the first
...
\roly{Might be best to deleted too and consider as subsumed into a new future work item on ``shape approximation''.}
...
\end{displaymath}
\Replaced{Compared to the interpretation of the untranslated query, which carries a single $\Two$ at each
number position and $\mathbb{1}$ elsewhere}{Comparing to the type in (\ref{eqn:reverse-query-approx-1})}, we
...
entry of the database. \Replaced{Without the translation}{With the previous interpretation}, we did not have
