...
\begin{example}
  ...
  \end{displaymath}
    As in differential calculus, the Jacobian is taken at a point: the two runs of the same program have
  different Jacobians. And like their numerical counterparts, these matrices compose: $\mathrm{total}\,l$
  ...
  $\partial_{\Two}$-entries are exactly their nonzero pattern: on multiplication, the dependency reading agrees with
  the derivative. The entry for $\mathit{price}\,\mathsf{a}$ is where they part company:
  ...
  instead records $1$: dependency information combines by joins, and joins cannot cancel. Boolean Jacobians
  are thus a \emph{sound over-approximation} of the nonzero pattern of the numerical ones, and this is
  inherent to tracking possible dependence.
