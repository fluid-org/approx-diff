...
\begin{example}
  ...
  \Added{However, sufficiency is only one way to read the forward direction. Reading the relation from inputs to outputs instead gives a second forward map, recording for each input position which parts of the output depend on it, and hence whether that input is \emph{necessary} to compute them: for $\mathit{query}\,\mathsf{a}$, the entry $(\mathsf{a},0)$ is necessary for the output, whereas $(\mathsf{b},1)$ is not. Whereas the sufficiency map is \emph{adjoint} to the backward map, forming a Galois connection, the necessity map is its \emph{conjugate}, a notion introduced by \citet{jonsson51}. For this particular choice of approximation, namely sets of positions within a value, the conjugate forward query is the De Morgan dual of the forwards Galois map~\cite{perera22}: an output depends on a given input exactly when it can no longer be reconstructed without it.}
