...
relating each output position to the input positions it depends on. This section works through a small
example showing how dependency relations behave as derivatives: they tabulate as Boolean matrices, they
compose like Jacobians, and they can be evaluated forwards or backwards.}{Perera and collaborators introduced
the idea of {\em Galois program slicing} as a particular conception of program slicing for provenance,
described in several publications~\cite{perera12a,perera16d,ricciotti17}. Galois program slicing (hereafter
simply {\emph \GPS}) forms the basis of the open source data visualisation tool
...
\begin{example}
  ...
  \Replaced{Suppose we are interested in which of the numerical parts of the input are used to compute the
  output, for the query parameters $l = \mathsf{a}$ and $l = \mathsf{b}$. The three numbers in $\mathit{db}$
  ...
  from the number at the second. We depict each output position pointing to the input positions it depends
  ...
  \end{displaymath}
  \cbend
  \Added{As in calculus, the Jacobian is taken at a point: the two runs of the same program have different
  Jacobians. And like their numerical counterparts, these matrices compose: $\mathit{query}\,l$ is a sum
  applied to a selection, and the Jacobian of the run is the product of the Jacobians of those two stages, a
  ...
  positions, it returns the output positions that depend on them. Writing
  $\partial(\mathit{query}\,l\,\mathit{db})_f$ for this forward action, we have
...
\Added{The backward maps compute \emph{program slices}: selecting part of the output yields the part of the
input needed to explain it, and when the program text is itself treated as part of the input, the relevant
part of the program too. One such technique is the \GPS of Perera and
collaborators~\cite{perera12a,perera16d,ricciotti17}, which underlies the open source data visualisation tool
\href{https://f.luid.org/}{Fluid}~\cite{perera2025fluid} and packages the forward and backward maps of a run
as a Galois connection between lattices of approximations. We do not take Galois connections as a starting
point; rather, \GPS will emerge later as one instance of the differential framework developed in this paper
