...
\begin{example}
  ...
  \begin{displaymath}
    \begin{array}{l}
      ...
      \mathrm{total}\,l\,\mathit{db}\,\mathit{price} = \mathit{price}\,l \cdot \mathrm{sum}\,[ q \mid (l',q) \leftarrow \mathit{db}, l \equiv l' ]
    \end{array}
  \end{displaymath}
  ...
  = 5$, we have $\mathrm{total}\,\mathsf{a}\,\mathit{db}\,\mathit{price} = 2 \cdot (3 - 3) = 0$ and
  ...
  is a price applied to a sum applied to a selection, and the Jacobian of the run is the product of the
  Jacobians of those stages, a chain rule for dependencies. For the run with $l = \mathsf{a}$:
  \begin{displaymath}
    \underbrace{\vphantom{\begin{pmatrix} 0 \\ 0 \\ 0 \end{pmatrix}}\begin{pmatrix} 2 & 0 \end{pmatrix}}_{\text{multiply}}
    \;
    \underbrace{\vphantom{\begin{pmatrix} 0 \\ 0 \\ 0 \end{pmatrix}}\begin{pmatrix} 1 & 1 & 0 \\ 0 & 0 & 1 \end{pmatrix}}_{\text{sum}}
    \;
  ...
  $\mathsf{a}$-quantities and $\mathit{price}\,\mathsf{a}$); the sum merges the two quantities, the price
  passing through; and the multiply stage is the Jacobian of the product at the point $(0, 2)$. The zeros of
  the composite row have visible origins: the entries for $(\mathsf{b},1)$ and $\mathit{price}\,\mathsf{b}$
  vanish at the selection stage, while the entry for $\mathit{price}\,\mathsf{a}$ vanishes at the multiply
  stage, where the cofactor is the cancelled sum.
  ...
  reading agrees with the derivative. The entry for $\mathit{price}\,\mathsf{a}$ is where they part company:
  ...
  instead records $1$: dependency information combines by disjunction, and disjunction cannot cancel. Boolean Jacobians
