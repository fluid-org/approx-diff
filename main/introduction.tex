...
they relate to the numerical Jacobians of differential calculus: agreement, except where arithmetic cancellation makes the
dependency reading a sound over-approximation}.
...
\begin{example}
  ...
  thus soundly over-approximate which entries of the numerical ones are nonzero, and this is
  inherent to tracking binary dependence.

  In setting up the positions, we chose to make only the numbers (quantities and prices)
  perturbable, keeping the labels and
  the structure of the list itself fixed. Other choices are also useful, and one of the aims of this work is
  to clarify how to choose a dependency structure appropriate for different tasks by selecting an appropriate
  semiring of dependency information. We elaborate on this further in
  \secref{semantic-gps}.
  ...
  \end{displaymath}
  for both $l = \mathsf{a}$ and $l = \mathsf{b}$. If instead we retain the output
  unapproximated, then the two
