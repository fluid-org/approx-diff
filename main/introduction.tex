...
backward slicing procedures generate Galois connections between these lattices. \Added{Galois connections
capture the backward direction well, recovering the least input responsible for a given output. Recent work
has shown that for data provenance, the natural forward slicing operation asks how each input affects the
output, and that this is better captured by an alternative construct called a \emph{conjugate pair}~\cite{jonsson51}. It is
here that a connection to differentiation emerges.} Our main contribution is \Added{to show} that this
approach can be seen as a generalised form of automatic differentiation, with slices of values playing the
role of tangents. Our categorical approach should provide a suitable setting for enabling ``automatic'' data
provenance for a variety of programming languages, and is easily configured to use alternative approximation
...
\Added{As \citet{bond25} show, however, underlying this Galois connection is a more elementary object: a \emph{dependency relation} between the parts of the input and the parts of the output, recording which inputs each output consumes. The Galois connection is just one possible derived view of this relation, obtained by lifting it to the lattices of approximations.}
...
\begin{example}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  These are not the only choices of approximation lattices that we could have made. For the input, we have chosen a lattice that allows us to ``forget'' (approximate away) numbers in the input, but not the labels or the structure of the list itself. \Added{Each such number occupies a \emph{position} in the value; an approximation keeps the shape of the value and retains the numbers only at some subset of the positions.} However, other choices are also useful. Indeed, one of the aims of this work is to clarify how to choose an approximation structure appropriate for different tasks by use of type information. We elaborate on this further in \secref{semantic-gps}.

  \GPS associates with each run of the program a Galois connection telling us how the inputs and outputs are related in that run. \Added{We can think of this as arising from a \emph{dependency relation} recording which input entries each part of the output depends on. Here the output is a single number, depending on the two $\mathsf{a}$-labelled entries for $\mathit{query}\,\mathsf{a}$, and the single $\mathsf{b}$-labelled one for $\mathit{query}\,\mathsf{b}$.}
  ...
  The backwards portion $\partial (\mathit{query}\,l)_r$ tells us, given an approximation of the output, what the least approximation of the input is needed to generate that output. \Added{This backwards map is simply the relation above lifted to sets: a set of output positions maps to the union of the input positions they depend on.} In the case of the two runs considered in this example, if we say we are not interested in the output by feeding in the least approximation $\bot$, \Added{namely the empty set of output positions,} then we find that we only need the least approximation of the input:
  \begin{displaymath}
  ...
  In this case, the forwards portion of the Galois connection tells us, for each approximation of the input, whether or not it is sufficient to compute the output. \Added{Here this just asks whether the input set contains every position the output set depends on.} If we provide insufficient data to compute the output, then we will get an underapproximated output. Here for example we will find that $\partial (\mathit{query}\, \mathsf{a})_f([(\mathsf{a},0),(\mathsf{b},\bot),(\mathsf{a},\bot)]) = \bot$ because we need all the values associated with the label $\mathsf{a}$ to compute their sum.

  \Added{However, sufficiency is only one way to read the forward direction. Reading the relation from inputs to outputs instead gives a second forward map, recording for each input position which parts of the output depend on it, and hence whether that input is \emph{necessary} to compute them. Writing $\partial(\mathit{query}\,\mathsf{a})_{\hat{f}}$ for this map, feeding in a single retained entry isolates its effect:}
  \cbstart
  \begin{displaymath}
    \begin{array}{l}
      \partial(\mathit{query}\,\mathsf{a})_{\hat{f}}([(\mathsf{a},0),(\mathsf{b},\bot),(\mathsf{a},\bot)]) = 1 \\
      \partial(\mathit{query}\,\mathsf{a})_{\hat{f}}([(\mathsf{a},\bot),(\mathsf{b},1),(\mathsf{a},\bot)]) = \bot
    \end{array}
  \end{displaymath}
  \cbend
  \Added{The first shows that $(\mathsf{a},0)$ is used, the second that $(\mathsf{b},1)$ is not; on the first input the sufficiency map returned $\bot$ instead, since one retained $\mathsf{a}$-entry is not enough to recompute the sum. Whereas the sufficiency map is \emph{adjoint} to the backward map, forming a Galois connection, the necessity map is its \emph{conjugate}, a notion introduced by \citet{jonsson51}. For this particular choice of approximation, namely sets of positions within a value, the conjugate forward query is the De Morgan dual of the forwards Galois map~\cite{perera22}: an output depends on a given input exactly when it can no longer be reconstructed without it.}
...
\item For \GPS, every program has an associated forward approximation map that \Replaced{takes approximations of the input to the least approximations of the output that depend on them}{takes approximations forward from the input to the output}. This map {\em preserves \Replaced{joins}{meets}}. For differentiable programs, every program has a forward derivative that takes tangents of the input to tangents of the output. The forward derivative map is {\em linear}, so it preserves addition of tangents and the zero tangent.
...
\item For \GPS, the forward and backward approximation maps are related by being \Replaced{a conjugate pair}{a Galois connection}. For differentiable programming, the forward and reverse derivatives are related by being each others' transpose.
\end{itemize}

\Added{In both settings the maps are \emph{decomposable} into atomic input-output facts that combine additively, so each is determined by its action on individual inputs. For automatic differentiation, this is exactly why the linear forward and reverse derivatives can be represented by Jacobian matrices, with composition given by matrix multiplication. For \GPS, as we shall see, these maps too can be represented by Jacobian matrices, now with Boolean scalars, where each entry records whether a given output depends on a given input.}
