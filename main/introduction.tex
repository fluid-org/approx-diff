...
\Added{As \citet{bond25} show, however, underlying this connection is a more elementary object: a \emph{dependency relation} between the parts of the input and the parts of the output, recording which inputs each output consumes. The Galois connection is just one possible derived view of this relation, obtained by lifting it to the lattices of approximations. The forward and backward directions are the two ways of reading it. We take the dependency relation as our starting point.}

This becomes {\em program slicing} by including the source code of the program as part of the input; then, in the backward direction, the least approximation of the input required for an output approximation includes the least part of the program required as well.
