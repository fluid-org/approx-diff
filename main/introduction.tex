...
\begin{example}
  ...
  \GPS associates with each run of the program a Galois connection telling us how the inputs and outputs are related in that run. \Added{We can think of this as arising from a \emph{dependency relation} recording which input entries the returned number depends on: for $\mathit{query}\,\mathsf{a}$, the two $\mathsf{a}$-labelled entries; for $\mathit{query}\,\mathsf{b}$, the single $\mathsf{b}$-labelled one.}
