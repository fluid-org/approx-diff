...
\to \GF(\mu_{\alpha.P})$, obtained by folding the $\GF$-image of the algebra, and not its
inverse}{only the comparison $\mu_{\alpha.\GF(P)}
...
Preservation asks only for an isomorphism of objects, so \Replaced{the required isomorphism can
be constructed}{the comparison can be carried out} on the
...
\Replaced{Together with $F$'s preservation of set-indexed coproducts, the lemma makes $\GF$
