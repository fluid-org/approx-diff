...
\Added{Glueing along a functor is the standard method for definability results, also known as
sconing \citep{mitchell-scedrov92}; \citet{fiore-simpson99} refine it with covers, which are needed
for sum types. \secref{predicate-system} sets up the calculus of predicates the construction uses,
...
\noindent
\Added{A predicate system is a first-order hyperdoctrine, with posets of predicates in place of
...
\Added{The functors assemble as}
\begin{AddedBlock}
\begin{center}
...
\end{AddedBlock}
\noindent
\Added{with $p \circ \GF = F$: an object of $\GLR(F)$ is a $\cat{D}$-object together with a closed
...
\Sigma\, \mu_{\alpha.\hat{P}}(\eta\,\delta)$\Added{, that is, the composite}
\begin{AddedBlock}
\begin{center}
...
\end{AddedBlock}
\noindent
\Added{which routes an environment through $\Fam(\cat{E})$, where the initial algebras exist by
