\RequirePackage{xr-hyper} % must precede hyperref (loaded by acmart)
...
% Resolve references into the companion document on inductive types
\externaldocument[mu:]{_latex/mu-types}[mu-types.pdf]
\newcommand*{\muSecref}[1]{\S\ref{mu:sec:#1}}
\newcommand*{\muDefref}[1]{Definition~\ref{mu:def:#1}}
\newcommand*{\muPropref}[1]{Proposition~\ref{mu:prop:#1}}
...
% \item slicing interpretation of the source language via a strong monad on the semantic category
\end{itemize}

\noindent Inductive types from polynomial endofunctors, previously covered here, now have their own
document (\texttt{mu-types.pdf}); references to that material resolve into it.
...
\input{notes/conservativity}
% \input{notes/slicing-interpretation}  % moved to unused: monadic slicing not load-bearing (see git history)
