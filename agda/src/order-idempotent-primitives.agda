...
open import cmon-enriched using (biproducts→products; Biproduct)
...
module over-biproducts (OI-BP : ∀ x y → Biproduct OI.cmon x y)
  (OI-BP-prod : ∀ x y → Biproduct.prod (OI-BP x y) ≡ (x OI.⊕ y)) where

  OI-products : HasProducts OI.cat
  OI-products = biproducts→products OI.cmon OI-BP
  ...
  module interp-primitives (Sig : Signature 0ℓ) (𝒫 : Primitives S Sig) where
    ...
    ≡-pos-mor : ∀ {P Q : OI.Pos} → P ≡ Q → OI._⇒_ P Q
    ≡-pos-mor {P} refl = OIC.id P
    ...
    collect (i ∷ is) =
      simplef[ idS _ , OI.disc-⊕ (sort-width i) (bases-width is) .fwd ]
        FC.∘ simplef[ idS _
                    , ≡-pos-mor (OI-BP-prod (OI.disc (sort-width i)) (OI.disc (bases-width is))) ]
        FC.∘ simple-monoidal
    ...
    module over (Ω : Obj) (into-Ω : Mor Fam⟨𝒞⟩-bool Ω) where
      ...
      private
        ...
        collect′ (i ∷ is) =
          simplef[ idS _ , OI.disc-⊕ (sort-width i) (bases-width is) .fwd ]
            FC.∘ simplef[ idS _
                        , ≡-pos-mor (OI-BP-prod (OI.disc (sort-width i))
                                                (OI.disc (bases-width is))) ]
            FC.∘ simple-monoidal
            FC.∘ FP.prod-m (FC.id (⟦sort⟧′ i)) (collect′ is)
