...
-- Paths of a control-source derivation, mirroring interaction.path.
module interaction.control-path {ℓ} (Sig : Signature ℓ) (𝒫 : Primitives two.semiring Sig) where
...
open import language-operational.evaluation Sig two.semiring 𝒫 two.I
  using (Val; Env; width; width-env; width-subst; lookup; bool→val; ctrl-row; proj-var; products)
open Val
open Env
...
mutual
  data Path : ∀ {Γ τ} {γ : Env Γ} {t : Γ ⊢ τ} {v : Val τ} {R : suc (width-env γ) ⇒ width v} →
              γ , t ⇓ v [ R ] → Set ℓ where
    ...
    roll    : ∀ {Γ} {τ : type 1} {γ : Env Γ} {t : Γ ⊢ τ [ μ τ ]} {v : Val (τ [ μ τ ])}
              {R : suc (width-env γ) ⇒ width v} {D : γ , t ⇓ v [ R ]} →
  ...
  data PathS : ∀ {Γ is} {γ : Env Γ} {Ms : Every (λ s → Γ ⊢ base s) is} {vs}
               {R : suc (width-env γ) ⇒ bases-width is} → γ , Ms ⇓s vs [ R ] → Set ℓ where
  ...
  data PathM : ∀ {Γ} {γ : Env Γ} {τ₀ : type 1} {σr : type 0} {s : Γ ▸ τ₀ [ σr ] ⊢ σr}
    ...
    m-rec₁  : ∀ {Γ} {γ : Env Γ} {τ₀ : type 1} {σr : type 0} {s : Γ ▸ τ₀ [ σr ] ⊢ σr}
              {w : Val (τ₀ [ μ τ₀ ])} {R : suc (width-env γ) ⇒ width w}
              {w' : Val (τ₀ [ σr ])} {R' : suc (width-env γ) ⇒ width w'}
              {u : Val σr} {S : suc (width-env (γ · w')) ⇒ width u}
    ...
    m-inl   : ∀ {Γ} {γ : Env Γ} {τ₀ : type 1} {σr : type 0} {s : Γ ▸ τ₀ [ σr ] ⊢ σr}
              {σ₁ σ₂ : type 1} {v : Val (σ₁ [ μ τ₀ ])} {R : suc (width-env γ) ⇒ suc (width v)}
              {v' : Val (σ₁ [ σr ])} {R' : suc (width-env γ) ⇒ width v'}
    ...
    m-pair₁ : ∀ {Γ} {γ : Env Γ} {τ₀ : type 1} {σr : type 0} {s : Γ ▸ τ₀ [ σr ] ⊢ σr}
              ...
              {R : suc (width-env γ) ⇒ width (pair v u)}
              {v' : Val (σ₁ [ σr ])} {S : suc (width-env γ) ⇒ width v'}
              {u' : Val (σ₂ [ σr ])} {T : suc (width-env γ) ⇒ width u'}
    ...
    m-mu    : ∀ {Γ} {γ : Env Γ} {τ₀ : type 1} {σr : type 0} {s : Γ ▸ τ₀ [ σr ] ⊢ σr}
              {τ' : type 2} {w : Val (unfold₁ τ' [ μ τ₀ ])} {R : suc (width-env γ) ⇒ width w}
              {w' : Val (unfold₁ τ' [ σr ])} {R' : suc (width-env γ) ⇒ width w'}
...
mutual
  eq-path-≡ : ∀ {Γ τ} {γ : Env Γ} {t : Γ ⊢ τ} {v R} {D : γ , t ⇓ v [ R ]}
              {p q : Path D} → eq-path p q ≡ Bool.true → p ≡ q
  eq-path-≡ {p = ε}         {ε}         h = ≡-refl
  eq-path-≡ {p = inl p}     {inl q}     h = ≡-cong inl (eq-path-≡ h)
  eq-path-≡ {p = inr p}     {inr q}     h = ≡-cong inr (eq-path-≡ h)
  eq-path-≡ {p = case-l₁ p} {case-l₁ q} h = ≡-cong case-l₁ (eq-path-≡ h)
  eq-path-≡ {p = case-l₂ p} {case-l₂ q} h = ≡-cong case-l₂ (eq-path-≡ h)
  eq-path-≡ {p = case-r₁ p} {case-r₁ q} h = ≡-cong case-r₁ (eq-path-≡ h)
  eq-path-≡ {p = case-r₂ p} {case-r₂ q} h = ≡-cong case-r₂ (eq-path-≡ h)
  eq-path-≡ {p = pair₁ p}   {pair₁ q}   h = ≡-cong pair₁ (eq-path-≡ h)
  eq-path-≡ {p = pair₂ p}   {pair₂ q}   h = ≡-cong pair₂ (eq-path-≡ h)
  eq-path-≡ {p = fst p}     {fst q}     h = ≡-cong fst (eq-path-≡ h)
  eq-path-≡ {p = snd p}     {snd q}     h = ≡-cong snd (eq-path-≡ h)
  eq-path-≡ {p = app₁ p}    {app₁ q}    h = ≡-cong app₁ (eq-path-≡ h)
  eq-path-≡ {p = app₂ p}    {app₂ q}    h = ≡-cong app₂ (eq-path-≡ h)
  eq-path-≡ {p = app₃ p}    {app₃ q}    h = ≡-cong app₃ (eq-path-≡ h)
  eq-path-≡ {p = bop p}     {bop q}     h = ≡-cong bop (eq-path-s-≡ h)
  eq-path-≡ {p = brel p}    {brel q}    h = ≡-cong brel (eq-path-s-≡ h)
  eq-path-≡ {p = roll p}    {roll q}    h = ≡-cong roll (eq-path-≡ h)
  eq-path-≡ {p = fold₁ p}   {fold₁ q}   h = ≡-cong fold₁ (eq-path-≡ h)
  eq-path-≡ {p = fold₂ p}   {fold₂ q}   h = ≡-cong fold₂ (eq-path-m-≡ h)
  eq-path-≡ {p = ε} {inl _} ()
  eq-path-≡ {p = inl _} {ε} ()
  eq-path-≡ {p = ε} {inr _} ()
  eq-path-≡ {p = inr _} {ε} ()
  eq-path-≡ {p = ε} {case-l₁ _} ()
  eq-path-≡ {p = case-l₁ _} {ε} ()
  eq-path-≡ {p = ε} {case-l₂ _} ()
  eq-path-≡ {p = case-l₂ _} {ε} ()
  eq-path-≡ {p = ε} {case-r₁ _} ()
  eq-path-≡ {p = case-r₁ _} {ε} ()
  eq-path-≡ {p = ε} {case-r₂ _} ()
  eq-path-≡ {p = case-r₂ _} {ε} ()
  eq-path-≡ {p = ε} {pair₁ _} ()
  eq-path-≡ {p = pair₁ _} {ε} ()
  ...
  eq-path-≡ {p = ε} {fold₂ _} ()
  eq-path-≡ {p = fold₂ _} {ε} ()
  eq-path-≡ {p = case-l₁ _} {case-l₂ _} ()
  eq-path-≡ {p = case-l₂ _} {case-l₁ _} ()
  eq-path-≡ {p = case-r₁ _} {case-r₂ _} ()
  eq-path-≡ {p = case-r₂ _} {case-r₁ _} ()
  eq-path-≡ {p = pair₁ _} {pair₂ _} ()
  eq-path-≡ {p = pair₂ _} {pair₁ _} ()
  eq-path-≡ {p = app₁ _} {app₂ _} ()
  eq-path-≡ {p = app₂ _} {app₁ _} ()
  eq-path-≡ {p = app₁ _} {app₃ _} ()
  eq-path-≡ {p = app₃ _} {app₁ _} ()
  eq-path-≡ {p = app₂ _} {app₃ _} ()
  eq-path-≡ {p = app₃ _} {app₂ _} ()
  eq-path-≡ {p = fold₁ _} {fold₂ _} ()
  eq-path-≡ {p = fold₂ _} {fold₁ _} ()

  eq-path-s-≡ : ∀ {Γ is} {γ : Env Γ} {Ms : Every (λ s → Γ ⊢ base s) is} {vs R}
                {D : γ , Ms ⇓s vs [ R ]} {p q : PathS D} → eq-path-s p q ≡ Bool.true → p ≡ q
  eq-path-s-≡ {p = ε}    {ε}    h = ≡-refl
  eq-path-s-≡ {p = hd p} {hd q} h = ≡-cong hd (eq-path-≡ h)
  eq-path-s-≡ {p = tl p} {tl q} h = ≡-cong tl (eq-path-s-≡ h)
  eq-path-s-≡ {p = ε} {hd _} ()
  eq-path-s-≡ {p = hd _} {ε} ()
  eq-path-s-≡ {p = ε} {tl _} ()
  eq-path-s-≡ {p = tl _} {ε} ()
  eq-path-s-≡ {p = hd _} {tl _} ()
  eq-path-s-≡ {p = tl _} {hd _} ()

  eq-path-m-≡ : ∀ {Γ} {γ : Env Γ} {τ₀ : type 1} {σr : type 0} {s : Γ ▸ τ₀ [ σr ] ⊢ σr}
                {σ' : type 1} {v : Val (σ' [ μ τ₀ ])} {R : suc (width-env γ) ⇒ width v}
                {v' : Val (σ' [ σr ])} {R' : suc (width-env γ) ⇒ width v'}
                {D : Map γ s σ' v R v' R'} {p q : PathM D} → eq-path-m p q ≡ Bool.true → p ≡ q
  eq-path-m-≡ {p = ε}         {ε}         h = ≡-refl
  eq-path-m-≡ {p = m-rec₁ p}  {m-rec₁ q}  h = ≡-cong m-rec₁ (eq-path-m-≡ h)
  eq-path-m-≡ {p = m-rec₂ p}  {m-rec₂ q}  h = ≡-cong m-rec₂ (eq-path-≡ h)
  eq-path-m-≡ {p = m-inl p}   {m-inl q}   h = ≡-cong m-inl (eq-path-m-≡ h)
  eq-path-m-≡ {p = m-inr p}   {m-inr q}   h = ≡-cong m-inr (eq-path-m-≡ h)
  eq-path-m-≡ {p = m-pair₁ p} {m-pair₁ q} h = ≡-cong m-pair₁ (eq-path-m-≡ h)
  eq-path-m-≡ {p = m-pair₂ p} {m-pair₂ q} h = ≡-cong m-pair₂ (eq-path-m-≡ h)
  eq-path-m-≡ {p = m-mu p}    {m-mu q}    h = ≡-cong m-mu (eq-path-m-≡ h)
  eq-path-m-≡ {p = ε} {m-rec₁ _} ()
  eq-path-m-≡ {p = m-rec₁ _} {ε} ()
  eq-path-m-≡ {p = ε} {m-rec₂ _} ()
  eq-path-m-≡ {p = m-rec₂ _} {ε} ()
  eq-path-m-≡ {p = ε} {m-inl _} ()
  eq-path-m-≡ {p = m-inl _} {ε} ()
  eq-path-m-≡ {p = ε} {m-inr _} ()
  eq-path-m-≡ {p = m-inr _} {ε} ()
  eq-path-m-≡ {p = ε} {m-pair₁ _} ()
  eq-path-m-≡ {p = m-pair₁ _} {ε} ()
  eq-path-m-≡ {p = ε} {m-pair₂ _} ()
  eq-path-m-≡ {p = m-pair₂ _} {ε} ()
  eq-path-m-≡ {p = ε} {m-mu _} ()
  eq-path-m-≡ {p = m-mu _} {ε} ()
