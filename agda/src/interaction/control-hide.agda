...
-- Hiding an intermediate of a control-source derivation, mirroring interaction.hide: each remaining
-- dependence relation absorbs the dependence routed through the hidden vertex. Hidden vertices stay
-- in the carrier, so hiding transforms the relations and the caller tracks which vertices are live;
-- relations at a hidden vertex are stale, never read. The source vertex is an input alongside env,
-- never hidden.
module interaction.control-hide {ℓ} (Sig : Signature ℓ) (𝒫 : Primitives two.semiring Sig) where
...
-- value is not first-order hidden, so that its live vertices are env, src, the root, and FO D.
...
collapse-s-src : ∀ {Γ is} {γ : Env Γ} {Ms : Every (λ s → Γ ⊢ base s) is} {vs R}
                 (D : γ , Ms ⇓s vs [ R ]) → M.Matrix (bases-width is) 1
collapse-s-src D = hide-all-s (graph-s D) (map at (paths-s D)) src (at ε)
...
-- The three collapses of a fold-action graph: the dependence of the result on the environment, on
-- the control source, and on the input value.
...
collapse-m-src : ∀ {Γ} {γ : Env Γ} {τ₀ : type 1} {σr : type 0} {s : Γ ▸ τ₀ [ σr ] ⊢ σr}
                 {σ' : type 1} {v : Val (σ' [ μ τ₀ ])} {R : suc (width-env γ) ⇒ width v}
                 {v' : Val (σ' [ σr ])} {R' : suc (width-env γ) ⇒ width v'}
                 (D : Map γ s σ' v R v' R') → M.Matrix (width v') 1
collapse-m-src D = hide-all-m (graph-m D) (map at (paths-m D)) src (at ε)
