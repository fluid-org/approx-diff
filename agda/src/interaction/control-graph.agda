...
-- Dependence graphs over a control-source derivation, mirroring interaction.graph: the source is an
-- input vertex alongside the environment. Collapsing is intended to recover the control-source
-- relation.
module interaction.control-graph {ℓ} (Sig : Signature ℓ) (𝒫 : Primitives two.semiring Sig) where
...
mutual
  ...
  graph (⇓-pair D₁ D₂) src (at (pair₁ q)) = graph D₁ src (at q)
  ...
  graph (⇓-brel D) src (at ε) = ctrl-row

  graph (⇓-roll D) env (at (roll q)) = graph D env (at q)
  graph (⇓-roll D) src (at (roll q)) = graph D src (at q)
  ...
  graph-m (m-rec D₁ D₂) src (at (m-rec₁ q)) = graph-m D₁ src (at q)
  ...
  graph-m (m-rec D₁ D₂) src (at (m-rec₂ q)) = graph D₂ src (at q)
  ...
  graph-m (m-inl D) src (at (m-inl q)) = graph-m D src (at q)
  ...
  graph-m (m-inl D) src (at ε) = src-root

  graph-m (m-inr D) env (at (m-inr q)) = graph-m D env (at q)
  graph-m (m-inr D) src (at (m-inr q)) = graph-m D src (at q)
  ...
  graph-m (m-inr D) src (at ε) = src-root

  graph-m (m-pair D₁ D₂) env (at (m-pair₁ q)) = graph-m D₁ env (at q)
  graph-m (m-pair D₁ D₂) src (at (m-pair₁ q)) = graph-m D₁ src (at q)
  ...
  graph-m (m-pair D₁ D₂) src (at ε) = src-root

  graph-m (m-mu {τ' = τ'} {w = w} D) env (at (m-mu q)) = graph-m D env (at q)
  graph-m (m-mu D) src (at (m-mu q)) = graph-m D src (at q)
