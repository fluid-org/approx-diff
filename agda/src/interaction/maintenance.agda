...
module _ {Γ τ} {γ : Env Γ} {t : Γ ⊢ τ} {v R} {D : γ , t ⇓ v [ R ]} where
  ...
  merge-region-resp : (G : Graph D) (w : Path D) {rss rss' : List (List (Path D))} →
  ...
  private
    adj : (G : Graph D) (w : Path D) → List (Path D) → Bool
    adj G w C = any (λ q → adjacent G (at w) (at q)) C

    merge-region-filter : (G : Graph D) (w : Path D) (rss : List (List (Path D))) →
  ...
  merge-region-comm : (G : Graph D) (w w' : Path D) (rss : List (List (Path D))) →
  ...
  regions-perm : (G : Graph D) {ws ws' : List (Path D)} → ws ↭ ws' →
  ...
  private
    ...
    regions-⊆ : (G : Graph D) (ws : List (Path D)) →
    ...
    merge-region-inert : (G : Graph D) (w : Path D) (X Y : List (List (Path D))) →
  ...
  regions-apart : (G : Graph D) (B rest : List (Path D)) → Apart G B rest →
  ...
  private
    apart-concat : {G : Graph D} {C : List (Path D)} {Cs : List (List (Path D))} →
    ...
    regions-nonempty : (G : Graph D) (ws : List (Path D)) →
  ...
  regions-apart-concat : {G : Graph D} {Cs : List (List (Path D))} →
                         AllPairs (Apart G) Cs →
