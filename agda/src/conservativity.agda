...
------------------------------------------------------------------------------
-- Glueing the rooted μ-types: the language of rooted families over the model,
-- with a family's predicates read at its total space through the collapse, so
-- the stages, covers and closure of the logical relations above are reused
-- unchanged. The root predicate picks out the definable root selections: the
-- maps that factor through a root at some index, up to cover refinement.
...
module rooted-mu
    (CM' : cmon-enriched.CMonEnriched 𝒟)
    (BP' : ∀ x y → cmon-enriched.Biproduct CM' x y)
    {𝟙d : Category.obj 𝒟} (Lft' : lifting.Lifting CM' 𝟙d)
    where

  module ΣD = fam-collapse 0ℓ 0ℓ 𝒟DC

  private
    module RML = fam-mu-lifting.laws 0ℓ 0ℓ 𝒟T CM' BP' Lft'

  -- The nerve of a family: the nerve of its total space.
  G∐ : Functor RML.cat PSh⟨𝒞⟩
  G∐ = G ∘F ΣD.Σf

  -- The singleton family at the lifting's unit.
  𝟙L : RML.Obj
  𝟙L = RML.simple[ prop-setoid.𝟙 {0ℓ} {0ℓ} , 𝟙d ]

  -- The bare root of a lifted family at an index.
  root-mor : ∀ (C : RML.Obj) (i : Setoid.Carrier (RML.idx C)) →
             RML.Mor 𝟙L (RML.Lf C)
  root-mor C i .RML.idxf .prop-setoid._⇒_.func _ = i
  root-mor C i .RML.idxf .prop-setoid._⇒_.func-resp-≈ _ =
    prop-setoid.IsEquivalence.refl (Setoid.isEquivalence (RML.idx C))
  root-mor C i .RML.famf .indexed-family._⇒f_.transf _ = RML.root
  root-mor C i .RML.famf .indexed-family._⇒f_.natural _ =
    RML.≈-trans RML.id-right (RML.≈-sym (RML.Lmap-root _))

  -- Definable root selections: up to cover refinement, the maps into a
  -- collapsed lifted family that factor through a bare root.
  Rt∐ : ∀ (C : RML.Obj) → PSh⟨𝒞⟩-system.Predicate (G∐ .fobj (RML.Lf C))
  Rt∐ C =
    CPred.𝐂 (PSh⟨𝒞⟩-system.⋁ (Setoid.Carrier (RML.idx C))
              (λ i → PSh⟨𝒞⟩-system._⟨_⟩ PSh⟨𝒞⟩-system.TT (G∐ .fmor (root-mor C i))))
    where module CPred = predicate-system.ClosureOp closureOp

  -- The glued rooted μ interface at the collapsed nerve.
  module RootedMu =
    fam-mu-lifting.glued-interface 𝒟T CM' BP' Lft'
