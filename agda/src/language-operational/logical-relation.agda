...
-- Fam(SemiMod(S)), in existential (computability) form. Defined by well-founded
...
Point : type 0 → Set
Point τ = Setoid.Carrier ((⟦ τ ⟧ty δ₀) .idx)
...
fibre-subst : (τ : type 0) {a a' : Point τ} → idx-≈ τ a a' → SM._⇒_ (Fibre τ a) (Fibre τ a')
...
roll-pt : (τ : type 1) → Point (τ [ μ τ ]) → Point (μ τ)
roll-pt τ a = roll-mor τ .idxf .prop-setoid._⇒_.func a

roll-fib : (τ : type 1) (a : Point (τ [ μ τ ])) →
           SM._⇒_ (Fibre (τ [ μ τ ]) a) (Fibre (μ τ) (roll-pt τ a))
...
pt-coerce : ∀ {σ σ' : type 0} → σ ≡ σ' → Point σ → Point σ'
pt-coerce e = ≡-subst Point e

fib-coerce : ∀ {σ σ' : type 0} (e : σ ≡ σ') (a : Point σ) →
             SM._⇒_ (Fibre σ a) (Fibre σ' (pt-coerce e a))
...
pt-in₁ : (σ τ : type 0) → Point σ → Point (σ [+] τ)
pt-in₁ σ τ a = FamCP.in₁ {⟦ σ ⟧ty δ₀} {⟦ τ ⟧ty δ₀} .idxf .prop-setoid._⇒_.func a

pt-in₂ : (σ τ : type 0) → Point τ → Point (σ [+] τ)
pt-in₂ σ τ a = FamCP.in₂ {⟦ σ ⟧ty δ₀} {⟦ τ ⟧ty δ₀} .idxf .prop-setoid._⇒_.func a

fib-in₁ : (σ τ : type 0) (a : Point σ) → SM._⇒_ (Fibre σ a) (Fibre (σ [+] τ) (pt-in₁ σ τ a))
fib-in₁ σ τ a = FamCP.in₁ {⟦ σ ⟧ty δ₀} {⟦ τ ⟧ty δ₀} .famf .indexed-family._⇒f_.transf a
  where import indexed-family

fib-in₂ : (σ τ : type 0) (a : Point τ) → SM._⇒_ (Fibre τ a) (Fibre (σ [+] τ) (pt-in₂ σ τ a))
fib-in₂ σ τ a = FamCP.in₂ {⟦ σ ⟧ty δ₀} {⟦ τ ⟧ty δ₀} .famf .indexed-family._⇒f_.transf a
  where import indexed-family

pt-p₁ : (σ τ : type 0) → Point (σ [×] τ) → Point σ
pt-p₁ σ τ ab = FamP.p₁ {⟦ σ ⟧ty δ₀} {⟦ τ ⟧ty δ₀} .idxf .prop-setoid._⇒_.func ab

pt-p₂ : (σ τ : type 0) → Point (σ [×] τ) → Point τ
pt-p₂ σ τ ab = FamP.p₂ {⟦ σ ⟧ty δ₀} {⟦ τ ⟧ty δ₀} .idxf .prop-setoid._⇒_.func ab

fib-p₁ : (σ τ : type 0) (ab : Point (σ [×] τ)) →
         SM._⇒_ (Fibre (σ [×] τ) ab) (Fibre σ (pt-p₁ σ τ ab))
fib-p₁ σ τ ab = FamP.p₁ {⟦ σ ⟧ty δ₀} {⟦ τ ⟧ty δ₀} .famf .indexed-family._⇒f_.transf ab
  where import indexed-family

fib-p₂ : (σ τ : type 0) (ab : Point (σ [×] τ)) →
         SM._⇒_ (Fibre (σ [×] τ) ab) (Fibre τ (pt-p₂ σ τ ab))
fib-p₂ σ τ ab = FamP.p₂ {⟦ σ ⟧ty δ₀} {⟦ τ ⟧ty δ₀} .famf .indexed-family._⇒f_.transf ab
  where import indexed-family

-- Application at points and the evaluation fibre map.
app-pt : (σ τ : type 0) → Point (σ [→] τ) → Point σ → Point τ
app-pt σ τ f a = FamE.eval .idxf .prop-setoid._⇒_.func (f , a)

∂ε : (σ τ : type 0) (f : Point (σ [→] τ)) (a : Point σ) →
     SM._⇒_ (Fam⟨𝒟⟩.fm ((FamP.prod (⟦ σ [→] τ ⟧ty δ₀) (⟦ σ ⟧ty δ₀)) .fam) (f , a))
            (Fibre τ (app-pt σ τ f a))
...
module WithAgreement
  ...
  data MuRel (τ₀ : type 1)
             ...
             (σ' : type 1) (v : Val (σ' [ μ τ₀ ])) (a : Point (σ' [ μ τ₀ ])) →
             Realiser (σ' [ μ τ₀ ]) v a → Set where
    mrel-roll  : ∀ {w a' r' a r} →
                 MuRel τ₀ Rel< τ₀ w a' r' →
                 Prf (∃ₚ (idx-≈ (μ τ₀) (roll-pt τ₀ a') a) λ e →
                      r ≈M (fibre-subst (μ τ₀) {roll-pt τ₀ a'} {a} e ∘M roll-fib τ₀ a' ∘M r')) →
    ...
    mrel-inl   : ∀ {σ₁ σ₂ : type 1} {v a' r' a r} →
                 MuRel τ₀ Rel< σ₁ v a' r' →
                 Prf (∃ₚ (idx-≈ ((σ₁ [+] σ₂) [ μ τ₀ ]) (pt-in₁ (σ₁ [ μ τ₀ ]) (σ₂ [ μ τ₀ ]) a') a) λ e →
                      r ≈M (fibre-subst ((σ₁ [+] σ₂) [ μ τ₀ ]) {pt-in₁ (σ₁ [ μ τ₀ ]) (σ₂ [ μ τ₀ ]) a'} {a} e ∘M fib-in₁ (σ₁ [ μ τ₀ ]) (σ₂ [ μ τ₀ ]) a' ∘M r')) →
  ...
  Rel-acc (σ [+] τ) (acc rs) (inl v) a r =
    Σ (Point σ) λ a' → Σ (Realiser σ v a') λ r' →
      ...
      Prf (∃ₚ (idx-≈ (σ [+] τ) (pt-in₁ σ τ a') a) λ e →
           r ≈M (fibre-subst (σ [+] τ) {pt-in₁ σ τ a'} {a} e ∘M fib-in₁ σ τ a' ∘M r'))
  Rel-acc (σ [+] τ) (acc rs) (inr v) a r =
    Σ (Point τ) λ a' → Σ (Realiser τ v a') λ r' →
      Rel-acc τ (rs (s≤s (m≤n+m (size τ) (size σ)))) v a' r' ×
      Prf (∃ₚ (idx-≈ (σ [+] τ) (pt-in₂ σ τ a') a) λ e →
           r ≈M (fibre-subst (σ [+] τ) {pt-in₂ σ τ a'} {a} e ∘M fib-in₂ σ τ a' ∘M r'))
  Rel-acc (σ [×] τ) (acc rs) (pair v u) a r =
    Rel-acc σ (rs (s≤s (m≤m+n (size σ) (size τ)))) v (pt-p₁ σ τ a)
            (fib-p₁ σ τ a ∘M r ∘M in-free₁ (width v) (width u)) ×
    Rel-acc τ (rs (s≤s (m≤n+m (size τ) (size σ)))) u (pt-p₂ σ τ a)
            (fib-p₂ σ τ a ∘M r ∘M in-free₂ (width v) (width u))
  Rel-acc (σ [→] τ) (acc rs) (clo {Γ'} γ' t) f r =
    ∀ (v : Val σ) (a : Point σ) (rv : Realiser σ v a) →
  ...
  PointC : ctxt → Set
  PointC Γ = Setoid.Carrier ((⟦ Γ ⟧ctxt) .idx)

  FibreC : (Γ : ctxt) → PointC Γ → SM.obj
  FibreC Γ g = Fam⟨𝒟⟩.fm ((⟦ Γ ⟧ctxt) .fam) g

  EnvRel : (Γ : ctxt) (γ : Env Γ) (g : PointC Γ) →
