...
-- Test derivative interpretation of `mult`.
module mult-deriv where
  open import categories using (Category; HasInitial; HasProducts)
  import cmon-enriched as CMon
  import matrix-new
  import semimodule
  import ho-model-matrix-new
  ...
  fwd : _ → _ → _ → _
  fwd x y = ⟦ example.ex.mult-ex ⟧tm .famf .transf (_ , (x , y)) .func
