...
open import categories
  using (Category; HasWeakExponentials; HasProducts; HasCoproducts)
open import indexed-family
  using (HasWeakSetoidProducts; Fam;
...
-- fam-exponentials with the extensionality laws dropped on both sides: if 𝒞 has binary
-- biproducts and weak Setoid-indexed products, then Fam(𝒞) has weak exponentials. The pairing
-- naturality that the currying's fibre naturality needs is only ever invoked at transports, which
-- are isomorphisms, so the weak products' restricted naturality suffices.

module fam-exponentials-weak {o m e} os es
   (𝒞 : Category o m e)
   (CM𝒞 : CMonEnriched 𝒞)
   (BP : ∀ x y → Biproduct CM𝒞 x y)
   (SP : HasWeakSetoidProducts (m ⊔ e ⊔ os ⊔ es) (os ⊔ es ⊔ m ⊔ e) 𝒞)
       where
...
open IsEquivalence
open HasWeakExponentials
open HasWeakSetoidProducts
open HasProducts
open HasCoproducts
...
lambda⟶ {X} {Y} {Z} f .famf .natural {x₁} {x₂} x₁≈x₂ =
  ...
  ≈⟨ SP .lambdaΠ-natural-iso _ {h⁻¹ = X .fam .subst (X .idx .isEquivalence .sym x₁≈x₂)}
       (isEquiv .trans (≈-sym (X .fam .trans* x₁≈x₂ (X .idx .isEquivalence .sym x₁≈x₂))) (X .fam .refl*))
       (isEquiv .trans (≈-sym (X .fam .trans* (X .idx .isEquivalence .sym x₁≈x₂) x₁≈x₂)) (X .fam .refl*)) ⟩
...
exponentials : HasWeakExponentials cat products
