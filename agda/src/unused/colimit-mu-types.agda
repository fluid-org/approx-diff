...
open polynomial-functor 𝒟T 𝒟P 𝒟SC using (Poly; extend; fobj; _∘co_; HasMu; HasMuLaws)
open Poly
  ...
  hasMuLaws .HasMuLaws.⦅⦆-η {Γ = Γ} {A = A} {P = P} {δ = δ} alg h eq =
    ...
    where
      ...
      h-leg (suc j) =
        ...
        (≈-trans (≈-sym (assoc _ _ _))
        (≈-trans (∘-cong₁ (∘-cong₂ (pair-cong id-left ≈-refl)))
        (≈-trans (∘-cong₂ (≈-trans (prod-m-cong (≈-sym id-left)
                                      (≈-trans (≈-sym id-left)
                                      (≈-trans (∘-cong₁ (≈-sym (≡-to-⇒-sym-l (⟦⟧-fobj P (extend δ (μ-carrier P δ))))))
                                               (assoc _ _ _))))
                                   (prod-m-comp _ _ _ _)))
        (≈-trans (≈-sym (assoc _ _ _))
