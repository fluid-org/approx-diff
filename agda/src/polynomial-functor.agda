  ...
  hasMu .HasMu.⦅⦆-η {Γ} {Q} {y} alg h h-step = result
    where
      ...
      η-fam (Poly.const A) γ j         = begin
          A .Obj.fam .Fam.subst _ ∘ p₂
        ≈⟨ ∘-cong (A .Obj.fam .Fam.refl*) ≈-refl ⟩
          id _ ∘ p₂
        ≈⟨ id-left ⟩
          p₂
        ≈⟨ ≈-sym (pair-p₂ _ _) ⟩
          p₂ ∘ pair p₁ p₂
        ≈⟨ ∘-cong ≈-refl (pair-cong ≈-refl (≈-sym id-left)) ⟩
          p₂ ∘ pair p₁ (id _ ∘ p₂)
        ∎ where open ≈-Reasoning isEquiv
      η-fam Poly.var       γ (inF j)   = begin
            ...
            (h .Mor.famf .transf (γ , inF (embed-idx Q (unembed-idx Q j))) ∘
             (Γ ⊗ WObj) .Obj.fam .Fam.subst
               (Γ .Obj.idx .isEquivalence .refl ,
                WObj .Obj.idx .isEquivalence .sym (embed-unembed-id Q j)))
        ≈⟨ ∘-cong ≈-refl (∘-cong ≈-refl
              (pair-cong (≈-trans (∘-cong (Γ .Obj.fam .Fam.refl*) ≈-refl) id-left) ≈-refl)) ⟩
          y .Obj.fam .Fam.subst proof-after ∘
            (h .Mor.famf .transf (γ , inF (embed-idx Q (unembed-idx Q j))) ∘
             pair p₁ (WFam-subst Q (WObj .Obj.idx .isEquivalence .sym (embed-unembed-id Q j)) ∘ p₂))
        ≈⟨ ∘-cong ≈-refl (∘-cong ≈-refl (pair-cong ≈-refl
              (∘-cong (≈-sym (embed-unembed-fam-id Q j)) ≈-refl))) ⟩
          y .Obj.fam .Fam.subst proof-after ∘
            (h .Mor.famf .transf (γ , inF (embed-idx Q (unembed-idx Q j))) ∘
        ...
        ≈⟨ ∘-cong ≈-refl (≈-sym (assoc _ _ _)) ⟩
          y .Obj.fam .Fam.subst proof-after ∘
            ((h .Mor.famf .transf (γ , inF (embed-idx Q (unembed-idx Q j))) ∘
              prod-m (id _) (embed-fam Q (unembed-idx Q j))) ∘
             pair p₁ (unembed-fam Q j ∘ p₂))
        ≈⟨ ∘-cong ≈-refl (∘-cong (∘-cong ≈-refl (pair-cong id-left ≈-refl)) ≈-refl) ⟩
          y .Obj.fam .Fam.subst proof-after ∘
            ((h .Mor.famf .transf (γ , inF (embed-idx Q (unembed-idx Q j))) ∘
        ...
        ≈⟨ isEquiv .trans (assoc _ _ _) (∘-cong ≈-refl (≈-sym (assoc _ _ _))) ⟩
          y .Obj.fam .Fam.subst alg-step ∘
            ((y .Obj.fam .Fam.subst h-step-bridge ∘
              (h .Mor.famf .transf (γ , inF (embed-idx Q (unembed-idx Q j))) ∘
               pair p₁ (embed-fam Q (unembed-idx Q j) ∘ p₂))) ∘
             pair p₁ (unembed-fam Q j ∘ p₂))
        ≈⟨ ∘-cong ≈-refl (∘-cong (∘-cong ≈-refl (∘-cong ≈-refl
          ...
          y .Obj.fam .Fam.subst alg-step ∘
            ((y .Obj.fam .Fam.subst h-step-bridge ∘
             ...
             pair p₁ (unembed-fam Q j ∘ p₂))
        ≈⟨ ∘-cong ≈-refl (∘-cong (∘-cong ≈-refl (≈-sym id-left)) ≈-refl) ⟩
          y .Obj.fam .Fam.subst alg-step ∘
            ((y .Obj.fam .Fam.subst h-step-bridge ∘
              (id _ ∘
              (h .Mor.famf .transf (γ , inF (embed-idx Q (unembed-idx Q j))) ∘
             ...
             pair p₁ (unembed-fam Q j ∘ p₂))
        ≈⟨ ∘-cong ≈-refl (∘-cong
              (h-step ._≃_.famf-eq .indexed-family._≃f_.transf-eq {γ , unembed-idx Q j})
              ≈-refl) ⟩
          y .Obj.fam .Fam.subst alg-step ∘
            ((cat Category.∘ alg)
               (products .HasProducts.pair (products .HasProducts.p₁) (poly-fmor Q h))
               .Mor.famf .transf (γ , unembed-idx Q j) ∘
             pair p₁ (unembed-fam Q j ∘ p₂))
        ≈⟨ ≈-refl ⟩
          y .Obj.fam .Fam.subst alg-step ∘
            ((id _ ∘
              (alg .Mor.famf .transf (γ , poly-fmor Q h .Mor.idxf .PS._⇒_.func (γ , unembed-idx Q j)) ∘
               pair p₁ (poly-fmor Q h .Mor.famf .transf (γ , unembed-idx Q j)))) ∘
             pair p₁ (unembed-fam Q j ∘ p₂))
        ...
        ≈⟨ isEquiv .trans (≈-sym (assoc _ _ _)) (∘-cong (≈-sym (assoc _ _ _)) ≈-refl) ⟩
          ((y .Obj.fam .Fam.subst alg-step ∘
           ...
           pair p₁ (poly-fmor Q h .Mor.famf .transf (γ , unembed-idx Q j))) ∘
           pair p₁ (unembed-fam Q j ∘ p₂)
        ≈˘⟨ ∘-cong (∘-cong (alg .Mor.famf .natural
              {γ , poly-fmor Q h .Mor.idxf .PS._⇒_.func (γ , unembed-idx Q j)}
              {γ , project-idx-open Q γ j}
              (Γ .Obj.idx .isEquivalence .refl ,
               η-idx Q (Γ .Obj.idx .isEquivalence .refl) (WIdx-≈-refl poly (idx-of Q)))) ≈-refl) ≈-refl ⟩
          ((alg .Mor.famf .transf (γ , project-idx-open Q γ j) ∘
            (Γ ⊗ poly-obj Q y) .Obj.fam .Fam.subst
              (Γ .Obj.idx .isEquivalence .refl ,
               η-idx Q (Γ .Obj.idx .isEquivalence .refl) (WIdx-≈-refl poly (idx-of Q)))) ∘
           pair p₁ (poly-fmor Q h .Mor.famf .transf (γ , unembed-idx Q j))) ∘
           pair p₁ (unembed-fam Q j ∘ p₂)
        ≈⟨ ∘-cong (∘-cong (∘-cong ≈-refl
              (pair-cong (≈-trans (∘-cong (Γ .Obj.fam .Fam.refl*) ≈-refl) id-left) ≈-refl)) ≈-refl) ≈-refl ⟩
          ((alg .Mor.famf .transf (γ , project-idx-open Q γ j) ∘
            pair p₁ (poly-obj Q y .Obj.fam .Fam.subst
                       (η-idx Q (Γ .Obj.idx .isEquivalence .refl) (WIdx-≈-refl poly (idx-of Q))) ∘ p₂)) ∘
           pair p₁ (poly-fmor Q h .Mor.famf .transf (γ , unembed-idx Q j))) ∘
           pair p₁ (unembed-fam Q j ∘ p₂)
        ...
        ≈⟨ ∘-cong ≈-refl (≈-sym (assoc _ _ _)) ⟩
          alg .Mor.famf .transf (γ , project-idx-open Q γ j) ∘
            ((pair p₁ (poly-obj Q y .Obj.fam .Fam.subst
             ...
             pair p₁ (unembed-fam Q j ∘ p₂))
        ≈⟨ ∘-cong ≈-refl (∘-cong
              (isEquiv .trans (pair-natural _ _ _)
        ...
        ≈⟨ ∘-cong ≈-refl (∘-cong (pair-cong ≈-refl
          ...
          alg .Mor.famf .transf (γ , project-idx-open Q γ j) ∘
            (pair p₁ ((poly-obj Q y .Obj.fam .Fam.subst project-bridge ∘
                      ...
                      poly-fmor Q h .Mor.famf .transf (γ , unembed-idx Q j)) ∘
             pair p₁ (unembed-fam Q j ∘ p₂))
        ≈⟨ ∘-cong ≈-refl (∘-cong (pair-cong ≈-refl (assoc _ _ _)) ≈-refl) ⟩
          alg .Mor.famf .transf (γ , project-idx-open Q γ j) ∘
            (pair p₁ (poly-obj Q y .Obj.fam .Fam.subst project-bridge ∘
                      (poly-obj Q y .Obj.fam .Fam.subst η-fam-Q-subst ∘
                       poly-fmor Q h .Mor.famf .transf (γ , unembed-idx Q j))) ∘
