...
-- The interpretation of the primitives in Fam(SDSemiMod(S)): a sort's fibre is the free object of its
-- width, and an operation's fibre map at a tuple of constants is its dependency relation there, read as
-- a morphism through the matrix embedding.
module primitives-interpretation
...
model .Model.⟦op⟧ {is} {o} ω = opω FC.∘ collect is
  ...
  opω .famf .natural {c} {c'} e =
    SM.≈-trans SM.id-right
      (SM.≈-trans (matrix-mor-cong (Category.≈-sym M.cat (op-rel ω .func-resp-≈ e))) (SM.≈-sym SM.id-left))
