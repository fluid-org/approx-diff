...
\paragraph{$\Poly$-types via realization.}
...
\GLR(F)$ are the realizations of those of $\Fam(\GLR(F))$: on presented polynomials, $\mu_{\alpha.P}
...
products to distribute over set-indexed coproducts (a consequence of $\GLR(F)$'s exponentials); and
the algebra map, catamorphism and $\beta$/$\eta$ laws must be transported along the resulting
isomorphisms.
...
(themselves realizations) is also what preservation of $\Poly$-types asks. Both assumptions thus
