...
\secref{conservativity:glueing}, only $\Poly$-types (\muDefref{polynomial-types:has-poly}), which we
...
(\muPropref{polynomial-types:fam-has-poly}); write $\sem{\tau}_{\mathit{fo}}$ for this interpretation. The
...
\muSecref{polynomial-types:fam}: realise families of $\GLR(F)$-objects as set-indexed coproducts, and
...
\muPropref{polynomial-types:fam-has-poly}, since $\cat{E}$ has a terminal object and finite
...
\muDefref{polynomial-types:has-poly}, $\Sigma$ must preserve what the construction uses:
...
strong coproducts. Then $\cat{E}$ has $\Poly$-types (\muDefref{polynomial-types:has-poly}).
\end{theorem}
...
concrete carriers of \muPropref{polynomial-types:fam-has-poly}, taking $\cat{C}$ to be a $\Fam$
